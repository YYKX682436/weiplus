package z51;

/* loaded from: classes10.dex */
public final class r extends z51.x {

    /* renamed from: f, reason: collision with root package name */
    public boolean f470211f;

    /* renamed from: g, reason: collision with root package name */
    public android.view.View f470212g;

    /* renamed from: h, reason: collision with root package name */
    public boolean f470213h;

    /* renamed from: i, reason: collision with root package name */
    public android.view.View f470214i;

    /* renamed from: j, reason: collision with root package name */
    public y51.f f470215j;

    /* renamed from: k, reason: collision with root package name */
    public int f470216k;

    /* renamed from: l, reason: collision with root package name */
    public int f470217l;

    /* renamed from: m, reason: collision with root package name */
    public int f470218m;

    /* renamed from: n, reason: collision with root package name */
    public int f470219n;

    /* renamed from: o, reason: collision with root package name */
    public int f470220o;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public r(y51.a pcContext) {
        super(pcContext);
        kotlin.jvm.internal.o.g(pcContext, "pcContext");
    }

    @Override // z51.x
    public java.lang.String c() {
        return "MircoMsg.LivePhotoTagViewPc";
    }

    @Override // z51.x
    public void d(a61.a config) {
        kotlin.jvm.internal.o.g(config, "config");
        super.d(config);
        java.lang.StringBuilder sb6 = new java.lang.StringBuilder("needTagView: ");
        sb6.append(this.f470211f);
        sb6.append(" has customTagView: ");
        sb6.append(this.f470212g != null);
        sb6.append(" fixedPosition: ");
        a61.d dVar = config.f1717a;
        sb6.append(dVar.f1726c);
        f(sb6.toString());
        this.f470211f = dVar.f1724a;
        this.f470212g = dVar.f1725b;
        this.f470213h = dVar.f1726c;
    }

    public final void k(int i17) {
        int b17;
        android.view.View view;
        f("calculateLocation >> bottomHeight: " + i17 + " locationDepends: " + this.f470216k + " thumbWidth: " + this.f470217l + " thumbHeight: " + this.f470218m + " startMargin:" + this.f470220o + " bottomMargin: " + this.f470219n + " tagViewIsFixedPosition: " + this.f470213h);
        if (!this.f470211f || this.f470215j == null || (b17 = i17 + i65.a.b(b(), 16)) == this.f470219n) {
            return;
        }
        com.tencent.mm.ui.ah a17 = com.tencent.mm.ui.bh.a(b());
        y51.f fVar = this.f470215j;
        kotlin.jvm.internal.o.d(fVar);
        int b18 = fVar.b();
        int i18 = a17.f197135a;
        int ceil = (int) java.lang.Math.ceil(((i18 * 1.0f) / this.f470217l) * this.f470218m);
        float f17 = ceil > b18 ? 0.0f : (b18 - ceil) / 2.0f;
        if (this.f470220o == 0) {
            if (ceil <= b18) {
                z51.p pVar = (z51.p) a(z51.p.class);
                int i19 = ceil + 2;
                pVar.f("changeVideoContainerHeight >> " + i19);
                android.widget.FrameLayout frameLayout = pVar.f470199f;
                if (frameLayout == null) {
                    kotlin.jvm.internal.o.o("videoViewContainer");
                    throw null;
                }
                android.view.ViewGroup.LayoutParams layoutParams = frameLayout.getLayoutParams();
                kotlin.jvm.internal.o.e(layoutParams, "null cannot be cast to non-null type android.widget.RelativeLayout.LayoutParams");
                android.widget.RelativeLayout.LayoutParams layoutParams2 = (android.widget.RelativeLayout.LayoutParams) layoutParams;
                layoutParams2.height = i19;
                android.widget.FrameLayout frameLayout2 = pVar.f470199f;
                if (frameLayout2 == null) {
                    kotlin.jvm.internal.o.o("videoViewContainer");
                    throw null;
                }
                frameLayout2.setLayoutParams(layoutParams2);
                this.f470220o = i65.a.b(b(), 16);
            } else {
                float f18 = ((this.f470217l * b18) / this.f470218m) * 1.0f;
                z51.p pVar2 = (z51.p) a(z51.p.class);
                int ceil2 = ((int) java.lang.Math.ceil(f18)) + 2;
                pVar2.f("changeVideoContainerWidth >> " + ceil2);
                android.widget.FrameLayout frameLayout3 = pVar2.f470199f;
                if (frameLayout3 == null) {
                    kotlin.jvm.internal.o.o("videoViewContainer");
                    throw null;
                }
                android.view.ViewGroup.LayoutParams layoutParams3 = frameLayout3.getLayoutParams();
                kotlin.jvm.internal.o.e(layoutParams3, "null cannot be cast to non-null type android.widget.RelativeLayout.LayoutParams");
                android.widget.RelativeLayout.LayoutParams layoutParams4 = (android.widget.RelativeLayout.LayoutParams) layoutParams3;
                layoutParams4.width = ceil2;
                android.widget.FrameLayout frameLayout4 = pVar2.f470199f;
                if (frameLayout4 == null) {
                    kotlin.jvm.internal.o.o("videoViewContainer");
                    throw null;
                }
                frameLayout4.setLayoutParams(layoutParams4);
                this.f470220o = i65.a.b(b(), 16);
            }
        }
        int b19 = (int) (f17 + i65.a.b(b(), 16));
        f("screenWidth: " + i18 + " showMaxHeight: " + b18 + " showHeight: " + ceil + " outSideSafeBottomMargin: " + b17 + " dependsThumbBottomMargin: " + b19 + " startMargin: " + this.f470220o);
        if (b19 > b17) {
            b17 = b19;
        }
        this.f470219n = b17;
        if (this.f470213h || (view = this.f470214i) == null) {
            return;
        }
        android.view.ViewGroup.LayoutParams layoutParams5 = view.getLayoutParams();
        kotlin.jvm.internal.o.e(layoutParams5, "null cannot be cast to non-null type android.widget.RelativeLayout.LayoutParams");
        android.widget.RelativeLayout.LayoutParams layoutParams6 = (android.widget.RelativeLayout.LayoutParams) layoutParams5;
        layoutParams6.setMarginStart(this.f470220o);
        layoutParams6.bottomMargin = this.f470219n;
        view.setLayoutParams(layoutParams6);
        this.f470216k = 1;
    }

    public android.widget.RelativeLayout.LayoutParams l() {
        android.widget.RelativeLayout.LayoutParams layoutParams = new android.widget.RelativeLayout.LayoutParams(-2, -2);
        layoutParams.addRule(12);
        if (this.f470213h) {
            layoutParams.setMarginStart(i65.a.b(b(), 16));
            layoutParams.bottomMargin = i65.a.b(b(), 16);
        }
        return layoutParams;
    }
}
