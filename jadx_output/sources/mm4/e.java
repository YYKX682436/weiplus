package mm4;

/* loaded from: classes14.dex */
public final class e extends mm4.g {

    /* renamed from: a, reason: collision with root package name */
    public final android.content.Context f329688a;

    /* renamed from: b, reason: collision with root package name */
    public int f329689b;

    /* renamed from: c, reason: collision with root package name */
    public final int f329690c;

    /* renamed from: d, reason: collision with root package name */
    public final int f329691d;

    /* renamed from: e, reason: collision with root package name */
    public final boolean f329692e;

    /* renamed from: f, reason: collision with root package name */
    public final jz5.g f329693f;

    /* renamed from: g, reason: collision with root package name */
    public final jz5.g f329694g;

    /* renamed from: h, reason: collision with root package name */
    public android.view.View f329695h;

    public e(android.content.Context context, int i17, int i18, int i19, boolean z17, int i27, kotlin.jvm.internal.i iVar) {
        i17 = (i27 & 2) != 0 ? com.tencent.mm.R.color.f478491c : i17;
        i18 = (i27 & 4) != 0 ? com.tencent.mm.R.color.BW_0_Alpha_0_5 : i18;
        i19 = (i27 & 8) != 0 ? com.tencent.mm.R.drawable.b6y : i19;
        z17 = (i27 & 16) != 0 ? true : z17;
        kotlin.jvm.internal.o.g(context, "context");
        this.f329688a = context;
        this.f329689b = i17;
        this.f329690c = i18;
        this.f329691d = i19;
        this.f329692e = z17;
        this.f329693f = jz5.h.b(new mm4.d(this));
        this.f329694g = jz5.h.b(new mm4.b(this));
        jz5.h.b(new mm4.c(this));
    }

    @Override // mm4.g
    public android.view.View a() {
        com.tencent.mm.ui.widget.imageview.WeImageView weImageView;
        android.view.View findViewById;
        android.view.View findViewById2;
        android.view.View view = this.f329695h;
        android.content.Context context = this.f329688a;
        if (view == null) {
            this.f329695h = com.tencent.mm.ui.id.b(context).inflate(com.tencent.mm.R.layout.f489579d15, (android.view.ViewGroup) null);
        }
        android.view.View view2 = this.f329695h;
        if (view2 != null && (findViewById2 = view2.findViewById(com.tencent.mm.R.id.f485216gy1)) != null) {
            findViewById2.setBackgroundResource(this.f329691d);
        }
        android.view.View view3 = this.f329695h;
        boolean z17 = this.f329692e;
        if (view3 != null && (findViewById = view3.findViewById(com.tencent.mm.R.id.gxy)) != null) {
            if (!z17) {
                findViewById.getLayoutParams().height = -2;
            }
            findViewById.setBackgroundColor(context.getResources().getColor(this.f329689b));
        }
        android.view.View view4 = this.f329695h;
        if (view4 != null && (weImageView = (com.tencent.mm.ui.widget.imageview.WeImageView) view4.findViewById(com.tencent.mm.R.id.gxz)) != null) {
            weImageView.setVisibility(z17 ? 0 : 8);
            weImageView.setIconColor(context.getResources().getColor(this.f329690c));
        }
        return this.f329695h;
    }

    @Override // mm4.g
    public void d(lm4.b0 layout) {
        kotlin.jvm.internal.o.g(layout, "layout");
        jz5.g gVar = this.f329693f;
        android.view.View view = (android.view.View) ((jz5.n) gVar).getValue();
        android.view.ViewGroup.LayoutParams layoutParams = ((android.view.View) ((jz5.n) gVar).getValue()).getLayoutParams();
        layoutParams.height = i65.a.b(this.f329688a, 4);
        view.setLayoutParams(layoutParams);
        ((com.tencent.mm.ui.widget.imageview.WeImageView) ((jz5.n) this.f329694g).getValue()).setAlpha(0.0f);
    }

    @Override // mm4.g
    public void e(lm4.b0 layout, float f17, java.lang.String source) {
        kotlin.jvm.internal.o.g(layout, "layout");
        kotlin.jvm.internal.o.g(source, "source");
        super.e(layout, f17, source);
        if (this.f329692e) {
            float borderExpand = layout.getBorderExpand();
            float borderExpand2 = layout.getBorderExpand();
            android.content.Context context = this.f329688a;
            float b17 = (f17 - borderExpand) / ((borderExpand2 + i65.a.b(context, 56)) - borderExpand);
            float f18 = (b17 - 0.5f) * 2.0f;
            if (f18 < 0.0f) {
                f18 = 0.0f;
            }
            if (f18 > 1.0f) {
                f18 = 1.0f;
            }
            if (b17 > 1.0f) {
                b17 = 1.0f;
            }
            float f19 = 0.0f < b17 ? b17 : 0.0f;
            int b18 = i65.a.b(context, 16);
            int b19 = i65.a.b(context, 4);
            jz5.g gVar = this.f329693f;
            android.view.View view = (android.view.View) ((jz5.n) gVar).getValue();
            android.view.ViewGroup.LayoutParams layoutParams = ((android.view.View) ((jz5.n) gVar).getValue()).getLayoutParams();
            layoutParams.height = (int) (b19 + (f19 * (b18 - b19)));
            view.setLayoutParams(layoutParams);
            ((com.tencent.mm.ui.widget.imageview.WeImageView) ((jz5.n) this.f329694g).getValue()).setAlpha(f18);
        }
    }

    public final void f(int i17) {
        android.view.View findViewById;
        this.f329689b = i17;
        android.view.View view = this.f329695h;
        if (view == null || (findViewById = view.findViewById(com.tencent.mm.R.id.gxy)) == null) {
            return;
        }
        findViewById.setBackgroundColor(this.f329688a.getResources().getColor(i17));
    }
}
