package uc1;

/* loaded from: classes13.dex */
public class r0 implements vc1.e2 {

    /* renamed from: a, reason: collision with root package name */
    public final /* synthetic */ int f426347a;

    /* renamed from: b, reason: collision with root package name */
    public final /* synthetic */ com.tencent.mm.plugin.appbrand.jsapi.t f426348b;

    public r0(uc1.i1 i1Var, int i17, com.tencent.mm.plugin.appbrand.jsapi.t tVar) {
        this.f426347a = i17;
        this.f426348b = tVar;
    }

    @Override // vc1.e2
    public void a(double d17, double d18) {
        uc1.z0 z0Var = new uc1.z0();
        org.json.JSONObject jSONObject = new org.json.JSONObject();
        try {
            jSONObject.put("mapId", this.f426347a);
            jSONObject.put("longitude", d17);
            jSONObject.put("latitude", d18);
        } catch (org.json.JSONException e17) {
            com.tencent.mars.xlog.Log.e("MicroMsg.JsApiInsertMap", "put JSON data error : %s", e17);
        }
        z0Var.f82374f = jSONObject.toString();
        this.f426348b.i(z0Var, null);
    }
}
