package iq5;

/* loaded from: classes5.dex */
public final class c implements android.view.View.OnClickListener {

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ com.tencent.mm.wedrop.ui.view.PermissionLineView f293805d;

    public c(com.tencent.mm.wedrop.ui.view.PermissionLineView permissionLineView) {
        this.f293805d = permissionLineView;
    }

    @Override // android.view.View.OnClickListener
    public final void onClick(android.view.View view) {
        yz5.a aVar;
        java.util.ArrayList arrayList = new java.util.ArrayList();
        arrayList.add(view);
        java.lang.Object[] array = arrayList.toArray();
        arrayList.clear();
        yj0.a.b("com/tencent/mm/wedrop/ui/view/PermissionLineView$checkPermission$2", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V", this, array);
        iq5.d viewModel = this.f293805d.getViewModel();
        if (viewModel != null && (aVar = viewModel.f293810h) != null) {
            aVar.invoke();
        }
        yj0.a.h(this, "com/tencent/mm/wedrop/ui/view/PermissionLineView$checkPermission$2", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V");
    }
}
