package ks2;

/* loaded from: classes8.dex */
public final class e extends androidx.recyclerview.widget.f2 {

    /* renamed from: d, reason: collision with root package name */
    public final java.util.ArrayList f311604d = new java.util.ArrayList();

    /* renamed from: e, reason: collision with root package name */
    public boolean f311605e;

    /* renamed from: f, reason: collision with root package name */
    public final /* synthetic */ ks2.j f311606f;

    public e(ks2.j jVar) {
        this.f311606f = jVar;
    }

    @Override // androidx.recyclerview.widget.f2
    public int getItemCount() {
        return this.f311604d.size();
    }

    @Override // androidx.recyclerview.widget.f2
    public int getItemViewType(int i17) {
        return ((r45.cq4) this.f311604d.get(i17)).getInteger(0);
    }

    @Override // androidx.recyclerview.widget.f2
    public void onBindViewHolder(androidx.recyclerview.widget.k3 holder, int i17) {
        android.text.SpannableString spannableString;
        kotlin.jvm.internal.o.g(holder, "holder");
        java.lang.Object obj = this.f311604d.get(i17);
        kotlin.jvm.internal.o.f(obj, "get(...)");
        r45.cq4 cq4Var = (r45.cq4) obj;
        boolean z17 = true;
        if (holder instanceof ks2.f) {
            this.f311605e = true;
            holder.itemView.setOnClickListener(new ks2.c(this, cq4Var));
            return;
        }
        if (holder instanceof ks2.g) {
            android.view.View view = holder.itemView;
            int i18 = this.f311605e ? 8 : 0;
            java.util.ArrayList arrayList = new java.util.ArrayList();
            java.lang.ThreadLocal threadLocal = zj0.c.f473285a;
            arrayList.add(java.lang.Integer.valueOf(i18));
            java.util.Collections.reverse(arrayList);
            yj0.a.d(view, arrayList.toArray(), "com/tencent/mm/plugin/finder/profile/moreInfo/ProfileMoreInfoVMDrawer$InfoItemDetailAdapter", "onBindViewHolder", "(Landroidx/recyclerview/widget/RecyclerView$ViewHolder;I)V", "android/view/View_EXEC_", "setVisibility", "(I)V");
            view.setVisibility(((java.lang.Integer) arrayList.get(0)).intValue());
            yj0.a.f(view, "com/tencent/mm/plugin/finder/profile/moreInfo/ProfileMoreInfoVMDrawer$InfoItemDetailAdapter", "onBindViewHolder", "(Landroidx/recyclerview/widget/RecyclerView$ViewHolder;I)V", "android/view/View_EXEC_", "setVisibility", "(I)V");
            ks2.g gVar = (ks2.g) holder;
            java.lang.String string = cq4Var.getString(1);
            android.widget.TextView textView = gVar.f311607d;
            textView.setText(string);
            java.lang.String string2 = cq4Var.getString(2);
            ks2.j jVar = this.f311606f;
            if (string2 != null) {
                i95.m c17 = i95.n0.c(zy2.s6.class);
                kotlin.jvm.internal.o.f(c17, "getService(...)");
                spannableString = zy2.s6.l6((zy2.s6) c17, string2, (int) jVar.E.getResources().getDimension(com.tencent.mm.R.dimen.f479575r), false, null, 0, false, null, 124, null);
            } else {
                spannableString = null;
            }
            android.widget.TextView textView2 = gVar.f311608e;
            textView2.setText(spannableString);
            java.lang.CharSequence text = textView.getText();
            if (text != null && text.length() != 0) {
                z17 = false;
            }
            if (z17) {
                textView.setVisibility(8);
                android.view.ViewGroup.LayoutParams layoutParams = textView2.getLayoutParams();
                android.view.ViewGroup.MarginLayoutParams marginLayoutParams = layoutParams instanceof android.view.ViewGroup.MarginLayoutParams ? (android.view.ViewGroup.MarginLayoutParams) layoutParams : null;
                if (marginLayoutParams != null) {
                    marginLayoutParams.setMargins(0, 0, 0, 0);
                }
                if (marginLayoutParams != null) {
                    textView2.setLayoutParams(marginLayoutParams);
                }
            } else {
                textView.setVisibility(0);
            }
            textView2.setTextColor(cq4Var.getInteger(0) == 2 ? jVar.E.getResources().getColor(com.tencent.mm.R.color.Link_100) : jVar.E.getResources().getColor(com.tencent.mm.R.color.abw));
            textView2.setOnClickListener(new ks2.d(cq4Var, holder));
            android.view.View view2 = gVar.f311609f;
            int i19 = cq4Var.getInteger(0) == 4 ? 0 : 8;
            java.util.ArrayList arrayList2 = new java.util.ArrayList();
            arrayList2.add(java.lang.Integer.valueOf(i19));
            java.util.Collections.reverse(arrayList2);
            yj0.a.d(view2, arrayList2.toArray(), "com/tencent/mm/plugin/finder/profile/moreInfo/ProfileMoreInfoVMDrawer$InfoItemDetailAdapter", "onBindViewHolder", "(Landroidx/recyclerview/widget/RecyclerView$ViewHolder;I)V", "android/view/View_EXEC_", "setVisibility", "(I)V");
            view2.setVisibility(((java.lang.Integer) arrayList2.get(0)).intValue());
            yj0.a.f(view2, "com/tencent/mm/plugin/finder/profile/moreInfo/ProfileMoreInfoVMDrawer$InfoItemDetailAdapter", "onBindViewHolder", "(Landroidx/recyclerview/widget/RecyclerView$ViewHolder;I)V", "android/view/View_EXEC_", "setVisibility", "(I)V");
        }
    }

    @Override // androidx.recyclerview.widget.f2
    public androidx.recyclerview.widget.k3 onCreateViewHolder(android.view.ViewGroup parent, int i17) {
        kotlin.jvm.internal.o.g(parent, "parent");
        ks2.j jVar = this.f311606f;
        if (i17 == 3) {
            android.view.View inflate = android.view.LayoutInflater.from(jVar.E).inflate(com.tencent.mm.R.layout.edq, parent, false);
            kotlin.jvm.internal.o.f(inflate, "inflate(...)");
            return new ks2.f(jVar, inflate);
        }
        android.view.View inflate2 = android.view.LayoutInflater.from(jVar.E).inflate(com.tencent.mm.R.layout.edr, parent, false);
        kotlin.jvm.internal.o.f(inflate2, "inflate(...)");
        return new ks2.g(jVar, inflate2);
    }
}
