package lp5;

/* loaded from: classes9.dex */
public abstract class a extends com.tencent.mm.wallet_core.model.d1 {

    /* renamed from: d, reason: collision with root package name */
    public com.tencent.mm.modelbase.u0 f320445d;

    /* renamed from: e, reason: collision with root package name */
    public com.tencent.mm.modelbase.o f320446e;

    public abstract java.lang.String H();

    public abstract int I();

    @Override // com.tencent.mm.modelbase.m1
    public int doScene(com.tencent.mm.network.s sVar, com.tencent.mm.modelbase.u0 u0Var) {
        this.f320445d = u0Var;
        return dispatch(sVar, this.f320446e, this);
    }

    @Override // com.tencent.mm.wallet_core.model.d1
    public void onGYNetEnd(int i17, int i18, int i19, java.lang.String str, com.tencent.mm.network.v0 v0Var, byte[] bArr, long j17) {
        java.lang.String string;
        java.lang.String str2;
        int i27;
        int i28 = i18;
        int i29 = i19;
        com.tencent.mars.xlog.Log.i("MiroMsg.NetSceneTenpayH5TransferBase", "Cmd : " + I() + ", errType = " + i28 + ", errCode = " + i29 + ", errMsg = " + str + " " + getType());
        r45.ex3 ex3Var = (r45.ex3) ((com.tencent.mm.modelbase.o) v0Var).f70711b.f70700a;
        if (i28 == 0 && i29 == 0) {
            int i37 = ex3Var.f373782e;
            java.lang.String f17 = x51.j1.f(ex3Var.f373781d);
            int i38 = 1000;
            if (i37 == 0 && !com.tencent.mm.sdk.platformtools.t8.K0(f17)) {
                int i39 = ex3Var.f373785h;
                try {
                    org.json.JSONObject jSONObject = new org.json.JSONObject(f17);
                    int i47 = jSONObject.getInt("retcode");
                    java.lang.String optString = jSONObject.optString("retmsg");
                    if (com.tencent.mm.sdk.platformtools.t8.K0(optString)) {
                        optString = ex3Var.f373786i;
                    }
                    if ((i47 == 0 && i39 == 0) || (this instanceof ww1.t1)) {
                        onGYNetEnd(i47, optString, jSONObject);
                        i38 = i28;
                        i39 = i29;
                        optString = str;
                    } else if (i39 == 0) {
                        i39 = -1000;
                    }
                    str2 = optString;
                    i27 = i39;
                } catch (java.lang.Exception e17) {
                    com.tencent.mars.xlog.Log.printErrStackTrace("MiroMsg.NetSceneTenpayH5TransferBase", e17, "", new java.lang.Object[0]);
                    str2 = com.tencent.mm.sdk.platformtools.x2.f193071a.getString(com.tencent.mm.R.string.kkk);
                }
                string = str2;
                i29 = i27;
                i28 = i38;
            } else {
                str2 = ex3Var.f373783f;
            }
            i27 = 2;
            string = str2;
            i29 = i27;
            i28 = i38;
        } else {
            java.util.Map d17 = com.tencent.mm.sdk.platformtools.aa.d(str, "e", null);
            if (d17 != null) {
                string = (java.lang.String) d17.get(".e.Content");
            } else {
                string = com.tencent.mm.sdk.platformtools.x2.f193071a.getString(com.tencent.mm.R.string.kkk);
            }
        }
        if (i28 != 0) {
            com.tencent.mars.xlog.Log.e("MiroMsg.NetSceneTenpayH5TransferBase", "Cmd : " + I() + ", errType = " + i28 + ", errCode = " + i29 + ", errMsg = " + string);
        }
        this.f320445d.onSceneEnd(i28, i29, string, this);
    }

    public abstract void onGYNetEnd(int i17, java.lang.String str, org.json.JSONObject jSONObject);

    public void setRequestData(java.util.Map map) {
        if (map == null) {
            map = new java.util.HashMap();
        }
        if (this.f320446e == null) {
            com.tencent.mm.modelbase.l lVar = new com.tencent.mm.modelbase.l();
            lVar.f70664a = new r45.dx3();
            lVar.f70665b = new r45.ex3();
            lVar.f70666c = H();
            lVar.f70667d = getType();
            lVar.f70668e = 0;
            lVar.f70669f = 0;
            com.tencent.mm.modelbase.o a17 = lVar.a();
            this.f320446e = a17;
            a17.setIsUserCmd(true);
        }
        r45.dx3 dx3Var = (r45.dx3) this.f320446e.f70710a.f70684a;
        dx3Var.f372823d = I();
        dx3Var.f372824e = 1;
        java.lang.Object[] array = map.keySet().toArray();
        java.util.Arrays.sort(array);
        java.lang.StringBuilder sb6 = new java.lang.StringBuilder();
        boolean z17 = false;
        for (java.lang.Object obj : array) {
            java.lang.String str = (java.lang.String) map.get(obj);
            if (!com.tencent.mm.sdk.platformtools.t8.K0(str)) {
                if (z17) {
                    sb6.append("&");
                }
                sb6.append(obj);
                sb6.append("=");
                sb6.append(str);
                z17 = true;
            }
        }
        com.tencent.mars.xlog.Log.i("MiroMsg.NetSceneTenpayH5TransferBase", "Cmd : " + dx3Var.f372823d + ", req = " + sb6.toString());
        byte[] bytes = sb6.toString().getBytes();
        r45.cu5 cu5Var = new r45.cu5();
        cu5Var.d(bytes);
        dx3Var.f372825f = cu5Var;
    }
}
