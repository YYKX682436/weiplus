package qg5;

/* loaded from: classes8.dex */
public final class o5 implements android.view.View.OnClickListener {

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ qg5.v5 f363117d;

    /* renamed from: e, reason: collision with root package name */
    public final /* synthetic */ db5.g3 f363118e;

    public o5(qg5.v5 v5Var, db5.g3 g3Var) {
        this.f363117d = v5Var;
        this.f363118e = g3Var;
    }

    @Override // android.view.View.OnClickListener
    public final void onClick(android.view.View view) {
        java.util.ArrayList arrayList = new java.util.ArrayList();
        arrayList.add(view);
        java.lang.Object[] array = arrayList.toArray();
        arrayList.clear();
        yj0.a.b("com/tencent/mm/ui/feature/api/YuanBaoSummarizeTosDialog$createDialog$1$1", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V", this, array);
        com.tencent.mars.xlog.Log.i("YuanBaoSummarizeTosDialog", "User accepted TOS");
        qg5.v5 v5Var = this.f363117d;
        v5Var.f363230e = true;
        com.tencent.mm.ui.widget.dialog.a4 a4Var = this.f363118e.f212055d;
        if (a4Var != null) {
            a4Var.dismiss();
        }
        yz5.a aVar = v5Var.f363227b;
        if (aVar != null) {
            aVar.invoke();
        }
        yj0.a.h(this, "com/tencent/mm/ui/feature/api/YuanBaoSummarizeTosDialog$createDialog$1$1", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V");
    }
}
