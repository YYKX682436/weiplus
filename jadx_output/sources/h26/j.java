package h26;

/* loaded from: classes16.dex */
public final class j extends f26.z0 {

    /* renamed from: e, reason: collision with root package name */
    public final f26.c2 f278343e;

    /* renamed from: f, reason: collision with root package name */
    public final y16.s f278344f;

    /* renamed from: g, reason: collision with root package name */
    public final h26.l f278345g;

    /* renamed from: h, reason: collision with root package name */
    public final java.util.List f278346h;

    /* renamed from: i, reason: collision with root package name */
    public final boolean f278347i;

    /* renamed from: m, reason: collision with root package name */
    public final java.lang.String[] f278348m;

    /* renamed from: n, reason: collision with root package name */
    public final java.lang.String f278349n;

    public j(f26.c2 constructor, y16.s memberScope, h26.l kind, java.util.List arguments, boolean z17, java.lang.String... formatParams) {
        kotlin.jvm.internal.o.g(constructor, "constructor");
        kotlin.jvm.internal.o.g(memberScope, "memberScope");
        kotlin.jvm.internal.o.g(kind, "kind");
        kotlin.jvm.internal.o.g(arguments, "arguments");
        kotlin.jvm.internal.o.g(formatParams, "formatParams");
        this.f278343e = constructor;
        this.f278344f = memberScope;
        this.f278345g = kind;
        this.f278346h = arguments;
        this.f278347i = z17;
        this.f278348m = formatParams;
        java.lang.Object[] copyOf = java.util.Arrays.copyOf(formatParams, formatParams.length);
        java.lang.String format = java.lang.String.format(kind.f278372d, java.util.Arrays.copyOf(copyOf, copyOf.length));
        kotlin.jvm.internal.o.f(format, "format(...)");
        this.f278349n = format;
    }

    @Override // f26.c3
    /* renamed from: B0 */
    public f26.c3 y0(g26.l kotlinTypeRefiner) {
        kotlin.jvm.internal.o.g(kotlinTypeRefiner, "kotlinTypeRefiner");
        return this;
    }

    @Override // f26.z0, f26.c3
    public f26.c3 C0(f26.r1 newAttributes) {
        kotlin.jvm.internal.o.g(newAttributes, "newAttributes");
        return this;
    }

    @Override // f26.z0
    /* renamed from: D0 */
    public f26.z0 A0(boolean z17) {
        f26.c2 c2Var = this.f278343e;
        y16.s sVar = this.f278344f;
        h26.l lVar = this.f278345g;
        java.util.List list = this.f278346h;
        java.lang.String[] strArr = this.f278348m;
        return new h26.j(c2Var, sVar, lVar, list, z17, (java.lang.String[]) java.util.Arrays.copyOf(strArr, strArr.length));
    }

    @Override // f26.z0
    /* renamed from: E0 */
    public f26.z0 C0(f26.r1 newAttributes) {
        kotlin.jvm.internal.o.g(newAttributes, "newAttributes");
        return this;
    }

    @Override // f26.o0
    public y16.s V() {
        return this.f278344f;
    }

    @Override // f26.o0
    public java.util.List u0() {
        return this.f278346h;
    }

    @Override // f26.o0
    public f26.r1 v0() {
        f26.r1.f259214e.getClass();
        return f26.r1.f259215f;
    }

    @Override // f26.o0
    public f26.c2 w0() {
        return this.f278343e;
    }

    @Override // f26.o0
    public boolean x0() {
        return this.f278347i;
    }

    @Override // f26.o0
    public f26.o0 y0(g26.l kotlinTypeRefiner) {
        kotlin.jvm.internal.o.g(kotlinTypeRefiner, "kotlinTypeRefiner");
        return this;
    }
}
