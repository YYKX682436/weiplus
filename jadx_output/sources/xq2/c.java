package xq2;

/* loaded from: classes2.dex */
public final class c extends kotlin.jvm.internal.q implements yz5.a {

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ vp2.c f456043d;

    /* renamed from: e, reason: collision with root package name */
    public final /* synthetic */ java.lang.String f456044e;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public c(vp2.c cVar, java.lang.String str) {
        super(0);
        this.f456043d = cVar;
        this.f456044e = str;
    }

    @Override // yz5.a
    public java.lang.Object invoke() {
        int i17;
        r45.xz2 xz2Var = new r45.xz2();
        r45.kv0 a17 = db2.t4.f228171a.a(10200);
        vp2.c cVar = this.f456043d;
        r45.le1 le1Var = cVar.f438910d;
        if (le1Var.B == 2) {
            i17 = 329;
        } else {
            r45.vz2 vz2Var = le1Var.f379358r;
            i17 = vz2Var != null && vz2Var.f388698p == 10 ? 260 : 261;
        }
        a17.set(1, java.lang.Integer.valueOf(i17));
        xz2Var.f390610d = a17;
        xz2Var.f390616m = 28;
        xz2Var.f390626w = cVar.f438911e.f374125d;
        com.tencent.mm.protobuf.g gVar = null;
        xz2Var.f390611e = null;
        xz2Var.f390628y = new java.util.LinkedList();
        xz2Var.B = this.f456044e;
        r45.le1 le1Var2 = cVar.f438910d;
        int i18 = le1Var2.f379348e;
        if (i18 == 4) {
            java.util.LinkedList anchor_group = le1Var2.f379362v;
            kotlin.jvm.internal.o.f(anchor_group, "anchor_group");
            r45.ne1 ne1Var = (r45.ne1) kz5.n0.a0(anchor_group, 0);
            if (ne1Var != null) {
                gVar = ne1Var.f381307e;
            }
        } else if (i18 == 5) {
            java.util.LinkedList product_group = le1Var2.f379363w;
            kotlin.jvm.internal.o.f(product_group, "product_group");
            r45.ue1 ue1Var = (r45.ue1) kz5.n0.a0(product_group, 0);
            if (ue1Var != null) {
                gVar = ue1Var.f387294g;
            }
        }
        xz2Var.D = gVar;
        xz2Var.E = le1Var2.f379365y;
        return (az2.j) xz2Var.b();
    }
}
