package gl5;

/* loaded from: classes15.dex */
public class w implements gl5.p {

    /* renamed from: d, reason: collision with root package name */
    public gl5.u f273105d;

    /* renamed from: e, reason: collision with root package name */
    public final /* synthetic */ gl5.e0 f273106e;

    public w(gl5.e0 e0Var) {
        this.f273106e = e0Var;
    }

    @Override // gl5.p
    public boolean a() {
        gl5.u uVar = this.f273105d;
        return uVar != null && uVar.k();
    }

    public gl5.u b() {
        if (this.f273105d == null) {
            gl5.e0 e0Var = this.f273106e;
            e0Var.w(false);
            this.f273105d = new gl5.u(e0Var, e0Var.I);
        }
        return this.f273105d;
    }

    @Override // gl5.p
    public boolean e() {
        gl5.u uVar = this.f273105d;
        return uVar != null && uVar.f273066o;
    }

    @Override // android.view.ViewTreeObserver.OnTouchModeChangeListener
    public void onTouchModeChanged(boolean z17) {
        gl5.u uVar;
        if (z17 || (uVar = this.f273105d) == null) {
            return;
        }
        uVar.i();
    }

    @Override // gl5.p
    public void show() {
        b().o();
        long uptimeMillis = android.os.SystemClock.uptimeMillis();
        float[] fArr = com.tencent.mm.ui.widget.cedit.edit.CustomTextView.f211571o2;
        long j17 = uptimeMillis - 0;
        gl5.e0 e0Var = this.f273106e;
        java.lang.Runnable runnable = e0Var.P;
        com.tencent.mm.ui.widget.cedit.edit.CustomTextView customTextView = e0Var.U;
        gl5.f0 f0Var = e0Var.O;
        if (runnable != null) {
            int i17 = f0Var.f272977e;
            if ((i17 == 2 || i17 == 3) || e0Var.u()) {
                customTextView.removeCallbacks(e0Var.P);
            }
        }
        int i18 = f0Var.f272977e;
        if (!(i18 == 2 || i18 == 3) && !e0Var.u() && j17 < 15000 && e0Var.t()) {
            if (e0Var.P == null) {
                e0Var.P = new gl5.v(this);
            }
            customTextView.postDelayed(e0Var.P, android.view.ViewConfiguration.getDoubleTapTimeout() + 1);
        }
        b().t();
        gl5.b0 b0Var = e0Var.f272944b;
        if (b0Var != null) {
            b0Var.c();
        }
    }
}
