package ji4;

/* loaded from: classes.dex */
public final class a0 implements c01.p8 {
    @Override // c01.p8
    public void a(java.lang.String str, java.lang.String str2, int i17) {
    }

    @Override // c01.p8
    public void b(java.lang.String str, int i17, c01.o8 o8Var) {
        if (str != null) {
            com.tencent.mars.xlog.Log.i("MicroMsg.TextStatus.TextStatusGetContactInterceptor", "getContact() called with: username = " + str + ", callback = " + o8Var);
        }
    }

    @Override // c01.p8
    public void d() {
    }

    @Override // c01.p8
    public void e(java.lang.String str) {
    }

    @Override // c01.p8
    public void f(java.lang.String str) {
    }

    @Override // c01.p8
    public void g(java.lang.String str, java.lang.String str2, int i17, c01.o8 o8Var) {
        if (str != null) {
            com.tencent.mars.xlog.Log.i("MicroMsg.TextStatus.TextStatusGetContactInterceptor", "getContact() called with: username = " + str + ", callback = " + o8Var);
        }
    }

    @Override // c01.p8
    public void h(java.lang.String str, int i17, java.lang.String str2) {
    }
}
