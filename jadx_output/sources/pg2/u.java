package pg2;

/* loaded from: classes3.dex */
public final class u implements ug5.i {

    /* renamed from: a, reason: collision with root package name */
    public android.view.ViewGroup f354186a;

    /* renamed from: b, reason: collision with root package name */
    public android.widget.ImageView f354187b;

    /* renamed from: c, reason: collision with root package name */
    public final /* synthetic */ android.view.View f354188c;

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ long f354189d;

    public u(android.view.View view, long j17) {
        this.f354188c = view;
        this.f354189d = j17;
    }

    @Override // ug5.i
    public void a(android.content.Context context, android.view.View view, android.widget.FrameLayout parent, ug5.v performer) {
        kotlin.jvm.internal.o.g(context, "context");
        kotlin.jvm.internal.o.g(view, "view");
        kotlin.jvm.internal.o.g(parent, "parent");
        kotlin.jvm.internal.o.g(performer, "performer");
        android.view.View view2 = this.f354188c;
        if (view2 != null) {
            view2.setTag(com.tencent.mm.R.id.tin, java.lang.Boolean.FALSE);
        }
        android.view.ViewGroup viewGroup = this.f354186a;
        if (viewGroup != null) {
            viewGroup.removeView(this.f354187b);
        }
        android.view.ViewParent parent2 = view2.getParent();
        android.view.ViewGroup viewGroup2 = parent2 instanceof android.view.ViewGroup ? (android.view.ViewGroup) parent2 : null;
        if (viewGroup2 != null) {
            viewGroup2.removeView(view2);
        }
        android.view.ViewGroup viewGroup3 = this.f354186a;
        if (viewGroup3 != null) {
            viewGroup3.addView(view2);
        }
        ig2.m mVar = ig2.m.f291393a;
        long j17 = this.f354189d;
        ig2.r f17 = mVar.f(j17);
        java.lang.StringBuilder sb6 = new java.lang.StringBuilder("onDetachPerformView feedId: ");
        sb6.append(j17);
        sb6.append(" director: ");
        sb6.append(f17);
        sb6.append(" isDetach: ");
        sb6.append(f17 != null ? java.lang.Boolean.valueOf(f17.i()) : null);
        com.tencent.mars.xlog.Log.i("FinderLiveSurfaceViewService", sb6.toString());
        boolean z17 = false;
        if (f17 != null && !f17.i()) {
            z17 = true;
        }
        if (z17) {
            f17.h();
        }
        super.a(context, view, parent, performer);
    }

    @Override // ug5.i
    public android.view.View b(android.content.Context context, android.widget.FrameLayout parent, ug5.v performer) {
        kotlin.jvm.internal.o.g(context, "context");
        kotlin.jvm.internal.o.g(parent, "parent");
        kotlin.jvm.internal.o.g(performer, "performer");
        android.view.View view = this.f354188c;
        android.view.ViewParent parent2 = view.getParent();
        android.view.ViewGroup viewGroup = parent2 instanceof android.view.ViewGroup ? (android.view.ViewGroup) parent2 : null;
        if (viewGroup != null) {
            if (view != null) {
                view.setTag(com.tencent.mm.R.id.tin, java.lang.Boolean.TRUE);
            }
            this.f354186a = viewGroup;
            viewGroup.removeView(view);
            android.widget.ImageView imageView = new android.widget.ImageView(context);
            imageView.setImageBitmap(ug5.x.b(view));
            imageView.setScaleType(android.widget.ImageView.ScaleType.CENTER_CROP);
            this.f354187b = imageView;
            viewGroup.addView(imageView, -1, -1);
            viewGroup.setVisibility(0);
        }
        return view;
    }
}
