package com.eclipsesource.mmv8;

/* loaded from: classes12.dex */
class LibraryLoader {
    static final java.lang.String DELIMITER = java.lang.System.getProperty("line.separator");
    static final java.lang.String SEPARATOR = java.lang.System.getProperty("file.separator");
    static final java.lang.String SWT_LIB_DIR = ".j2v8";
    private byte _hellAccFlag_;

    public static void chmod(java.lang.String str, java.lang.String str2) {
        if (isWindows()) {
            return;
        }
        try {
            java.lang.Runtime.getRuntime().exec(new java.lang.String[]{"chmod", str, str2}).waitFor();
        } catch (java.lang.Throwable unused) {
        }
    }

    private static java.lang.String computeLibraryFullName() {
        return com.tencent.tinker.loader.shareutil.ShareConstants.SO_PATH + computeLibraryShortName() + "." + getOSFileExtension();
    }

    private static java.lang.String computeLibraryShortName() {
        return "j2v8_" + getOS() + "_" + getArchSuffix();
    }

    public static boolean extract(java.lang.String str, java.lang.String str2, java.lang.StringBuffer stringBuffer) {
        java.io.InputStream inputStream;
        boolean z17;
        java.io.File file = new java.io.File(str);
        java.io.FileOutputStream fileOutputStream = null;
        try {
            if (file.exists()) {
                file.delete();
            }
            inputStream = com.eclipsesource.mmv8.LibraryLoader.class.getResourceAsStream("/" + str2);
            if (inputStream != null) {
                z17 = true;
                try {
                    byte[] bArr = new byte[4096];
                    java.io.FileOutputStream fileOutputStream2 = new java.io.FileOutputStream(str);
                    while (true) {
                        try {
                            int read = inputStream.read(bArr);
                            if (read == -1) {
                                break;
                            }
                            fileOutputStream2.write(bArr, 0, read);
                        } catch (java.lang.Throwable unused) {
                            fileOutputStream = fileOutputStream2;
                            if (fileOutputStream != null) {
                                try {
                                    fileOutputStream.close();
                                } catch (java.io.IOException unused2) {
                                }
                            }
                            if (inputStream != null) {
                                try {
                                    inputStream.close();
                                } catch (java.io.IOException unused3) {
                                }
                            }
                            if (z17 && file.exists()) {
                                file.delete();
                            }
                            return false;
                        }
                    }
                    fileOutputStream2.close();
                    inputStream.close();
                    chmod("755", str);
                    if (load(str, stringBuffer)) {
                        return true;
                    }
                } catch (java.lang.Throwable unused4) {
                }
            }
        } catch (java.lang.Throwable unused5) {
            inputStream = null;
            z17 = false;
        }
        return false;
    }

    public static java.lang.String getArchSuffix() {
        java.lang.String property = java.lang.System.getProperty("os.arch");
        return property.equals("i686") ? "x86" : property.equals("amd64") ? "x86_64" : (property.equals(com.eclipsesource.mmv8.Platform.NATIVE_CLIENT) || property.equals("aarch64")) ? "armv7l" : property;
    }

    public static java.lang.String getOS() {
        if (isWindows()) {
            return "win32";
        }
        if (isMac()) {
            return com.eclipsesource.mmv8.Platform.MACOSX;
        }
        if (isLinux() && !isAndroid()) {
            return com.eclipsesource.mmv8.Platform.LINUX;
        }
        if (isAndroid()) {
            return com.eclipsesource.mmv8.Platform.ANDROID;
        }
        throw new java.lang.UnsatisfiedLinkError("Unsupported platform: " + getOsName());
    }

    public static java.lang.String getOSFileExtension() {
        if (isWindows()) {
            return "dll";
        }
        if (isMac()) {
            return "dylib";
        }
        if (isLinux() || isNativeClient()) {
            return "so";
        }
        throw new java.lang.UnsatisfiedLinkError("Unsupported platform: " + getOsName());
    }

    public static java.lang.String getOsName() {
        return java.lang.System.getProperty("os.name") + java.lang.System.getProperty("java.specification.vendor");
    }

    public static boolean isAndroid() {
        return getOsName().contains("Android");
    }

    public static boolean isLinux() {
        return getOsName().startsWith("Linux");
    }

    public static boolean isMac() {
        return getOsName().startsWith("Mac");
    }

    public static boolean isNativeClient() {
        return getOsName().startsWith(com.eclipsesource.mmv8.Platform.NATIVE_CLIENT);
    }

    public static boolean isWindows() {
        return getOsName().startsWith("Windows");
    }

    public static boolean load(java.lang.String str, java.lang.StringBuffer stringBuffer) {
        try {
            if (str.indexOf(SEPARATOR) != -1) {
                com.tencent.cso.CsoLoader csoLoader = com.tencent.cso.CsoLoader.f45934a;
                com.tencent.cso.CsoLoader.h(str);
                return true;
            }
            java.util.ArrayList arrayList = new java.util.ArrayList();
            arrayList.add(str);
            java.lang.Object obj = new java.lang.Object();
            java.util.Collections.reverse(arrayList);
            yj0.a.d(obj, arrayList.toArray(), "com/eclipsesource/mmv8/LibraryLoader", "load", "(Ljava/lang/String;Ljava/lang/StringBuffer;)Z", "java/lang/System_EXEC_", "loadLibrary", "(Ljava/lang/String;)V");
            com.tencent.cso.CsoLoader.e((java.lang.String) arrayList.get(0));
            yj0.a.f(obj, "com/eclipsesource/mmv8/LibraryLoader", "load", "(Ljava/lang/String;Ljava/lang/StringBuffer;)Z", "java/lang/System_EXEC_", "loadLibrary", "(Ljava/lang/String;)V");
            return true;
        } catch (java.lang.UnsatisfiedLinkError e17) {
            if (stringBuffer.length() == 0) {
                stringBuffer.append(DELIMITER);
            }
            stringBuffer.append('\t');
            stringBuffer.append(e17.getMessage());
            stringBuffer.append(DELIMITER);
            return false;
        }
    }

    public static void loadLibrary(java.lang.String str) {
        if (isAndroid()) {
            java.util.ArrayList arrayList = new java.util.ArrayList();
            arrayList.add("mmj2v8");
            java.lang.Object obj = new java.lang.Object();
            java.util.Collections.reverse(arrayList);
            yj0.a.d(obj, arrayList.toArray(), "com/eclipsesource/mmv8/LibraryLoader", "loadLibrary", "(Ljava/lang/String;)V", "java/lang/System_EXEC_", "loadLibrary", "(Ljava/lang/String;)V");
            com.tencent.cso.CsoLoader.e((java.lang.String) arrayList.get(0));
            yj0.a.f(obj, "com/eclipsesource/mmv8/LibraryLoader", "loadLibrary", "(Ljava/lang/String;)V", "java/lang/System_EXEC_", "loadLibrary", "(Ljava/lang/String;)V");
            return;
        }
        java.lang.StringBuffer stringBuffer = new java.lang.StringBuffer();
        java.lang.String computeLibraryShortName = computeLibraryShortName();
        java.lang.String computeLibraryFullName = computeLibraryFullName();
        java.lang.StringBuilder sb6 = new java.lang.StringBuilder();
        sb6.append(java.lang.System.getProperty("user.dir"));
        java.lang.String str2 = SEPARATOR;
        sb6.append(str2);
        sb6.append("jni");
        sb6.append(str2);
        sb6.append(computeLibraryFullName());
        java.lang.String sb7 = sb6.toString();
        if (load(computeLibraryFullName, stringBuffer) || load(computeLibraryShortName, stringBuffer)) {
            return;
        }
        if (new java.io.File(sb7).exists() && load(sb7, stringBuffer)) {
            return;
        }
        if (str == null) {
            str = java.lang.System.getProperty("java.io.tmpdir");
        }
        if (extract(str + str2 + computeLibraryFullName, computeLibraryFullName, stringBuffer)) {
            return;
        }
        throw new java.lang.UnsatisfiedLinkError("Could not load J2V8 library. Reasons: " + stringBuffer.toString());
    }
}
