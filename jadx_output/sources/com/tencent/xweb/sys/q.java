package com.tencent.xweb.sys;

/* loaded from: classes13.dex */
public class q implements tx5.i {
    public q(com.tencent.xweb.sys.l lVar) {
    }

    @Override // tx5.i
    public tx5.g a() {
        return null;
    }

    @Override // tx5.i
    public tx5.f b(com.tencent.xweb.WebView webView) {
        try {
            return new com.tencent.xweb.sys.v(webView);
        } catch (java.lang.Throwable th6) {
            java.lang.String message = th6.getMessage();
            if (message != null && message.contains("Calling View methods on another thread than the UI thread.")) {
                by5.c4.c("SysWebFactory", "createWebView error:" + th6);
                boolean z17 = com.tencent.xweb.sys.o.f220565a;
                by5.d4.a("SysWebFactory.PreIniter", false).edit().putBoolean("isEnforceMainLooper", true).apply();
            }
            throw th6;
        }
    }

    @Override // tx5.i
    public boolean c(android.content.Context context, com.tencent.xweb.e1 e1Var) {
        by5.c4.f("SysWebFactory", "initWebviewCore");
        boolean z17 = com.tencent.xweb.sys.o.f220565a;
        synchronized (com.tencent.xweb.sys.o.class) {
            if (com.tencent.xweb.sys.o.f220565a) {
                by5.c4.f("SysWebFactory.PreIniter", "preInit, has already init");
            } else if (!com.tencent.xweb.sys.o.b()) {
                by5.c4.f("SysWebFactory.PreIniter", "preInit, not enforce main looper");
                com.tencent.xweb.sys.o.f220565a = true;
                if (e1Var != null) {
                    e1Var.a();
                }
            } else if (com.tencent.xweb.sys.o.f220567c.get()) {
                by5.c4.f("SysWebFactory.PreIniter", "preInit, enforce main looper & already called");
                if (e1Var != null) {
                    e1Var.a();
                }
            } else {
                by5.c4.f("SysWebFactory.PreIniter", "preInit, enforce main looper & not yet called");
                com.tencent.xweb.sys.n nVar = new com.tencent.xweb.sys.n(e1Var);
                if (android.os.Looper.getMainLooper() == android.os.Looper.myLooper()) {
                    nVar.run();
                } else {
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(nVar);
                }
            }
        }
        return true;
    }

    @Override // tx5.i
    public boolean d() {
        return com.tencent.xweb.sys.o.f220565a;
    }

    @Override // tx5.i
    public tx5.e e() {
        boolean z17 = com.tencent.xweb.o2.c() && y4.n.a(org.chromium.support_lib_boundary.util.Features.MULTI_PROFILE);
        if ((!com.tencent.xweb.sys.o.b() ? true : com.tencent.xweb.sys.o.f220567c.get()) && !z17) {
            by5.c4.f("SysWebFactory", "getWebviewStorage, already init");
            return i();
        }
        try {
            return (tx5.e) by5.a1.e(new com.tencent.xweb.sys.m(this));
        } catch (java.lang.Throwable th6) {
            by5.c4.d("SysWebFactory", "getCookieManager error", th6);
            xx5.l.a(4, 1, z17);
            return i();
        }
    }

    @Override // tx5.i
    public void f(android.content.Context context, boolean z17) {
        try {
            android.webkit.WebView webView = new android.webkit.WebView(context);
            webView.removeJavascriptInterface("searchBoxJavaBridge_");
            webView.removeJavascriptInterface("accessibility");
            webView.removeJavascriptInterface("accessibilityTraversal");
            webView.clearCache(true);
            android.webkit.WebViewDatabase.getInstance(context).clearUsernamePassword();
            android.webkit.WebViewDatabase.getInstance(context).clearHttpAuthUsernamePassword();
            android.webkit.WebViewDatabase.getInstance(context).clearFormData();
            android.webkit.WebIconDatabase.getInstance().removeAllIcons();
            if (z17) {
                android.webkit.CookieSyncManager.createInstance(context);
                tx5.b cookieManager = getCookieManager();
                if (cookieManager != null) {
                    cookieManager.e();
                }
            }
            tx5.e e17 = e();
            if (e17 != null) {
                e17.a();
            }
        } catch (java.lang.Throwable th6) {
            by5.c4.d("SysWebFactory", "clearAllWebViewCache error", th6);
        }
    }

    @Override // tx5.i
    public tx5.h g() {
        return null;
    }

    @Override // tx5.i
    public tx5.b getCookieManager() {
        boolean z17 = com.tencent.xweb.o2.c() && y4.n.a(org.chromium.support_lib_boundary.util.Features.MULTI_PROFILE);
        if ((!com.tencent.xweb.sys.o.b() ? true : com.tencent.xweb.sys.o.f220567c.get()) && !z17) {
            by5.c4.f("SysWebFactory", "getCookieManager, already init");
            return h();
        }
        try {
            return (tx5.b) by5.a1.e(new com.tencent.xweb.sys.l(this));
        } catch (java.lang.Throwable th6) {
            by5.c4.d("SysWebFactory", "getCookieManager error", th6);
            xx5.l.a(3, 1, z17);
            return h();
        }
    }

    @Override // tx5.i
    public tx5.d getProfileStore() {
        by5.a1.a();
        return new com.tencent.xweb.sys.c();
    }

    public final tx5.b h() {
        boolean z17 = com.tencent.xweb.o2.c() && y4.n.a(org.chromium.support_lib_boundary.util.Features.MULTI_PROFILE);
        if (z17) {
            by5.c4.f("SysWebFactory", "getCookieManagerInner, profileName:" + com.tencent.xweb.o2.f220368c);
            tx5.c orCreateProfile = com.tencent.xweb.c0.a().getOrCreateProfile(com.tencent.xweb.o2.f220368c);
            if (orCreateProfile != null) {
                return orCreateProfile.getCookieManager();
            }
            by5.c4.c("SysWebFactory", "getCookieManagerInner error, profile is null, Name: " + com.tencent.xweb.o2.f220368c);
            xx5.l.a(3, 1, z17);
            return null;
        }
        by5.c4.f("SysWebFactory", "getCookieManagerInner, isSupportMultiProfile: " + com.tencent.xweb.o2.c() + ", isFeatureSupported: " + y4.n.a(org.chromium.support_lib_boundary.util.Features.MULTI_PROFILE));
        try {
            return new com.tencent.xweb.sys.a();
        } catch (java.lang.Exception e17) {
            by5.c4.c("SysWebFactory", "getCookieManager error, sys CookieManager#getInstance error: " + e17);
            xx5.l.a(3, 1, z17);
            return null;
        }
    }

    public final tx5.e i() {
        boolean z17 = com.tencent.xweb.o2.c() && y4.n.a(org.chromium.support_lib_boundary.util.Features.MULTI_PROFILE);
        if (z17) {
            by5.c4.f("SysWebFactory", "getWebviewStorageInner, profileName:" + com.tencent.xweb.o2.f220368c);
            tx5.c orCreateProfile = com.tencent.xweb.c0.a().getOrCreateProfile(com.tencent.xweb.o2.f220368c);
            if (orCreateProfile != null) {
                return orCreateProfile.getWebStorage();
            }
            by5.c4.c("SysWebFactory", "getWebviewStorage error, profile is null, Name: " + com.tencent.xweb.o2.f220368c);
            xx5.l.a(4, 1, z17);
            return null;
        }
        by5.c4.f("SysWebFactory", "getWebviewStorage, isSupportMultiProfile: " + com.tencent.xweb.o2.c() + ", isFeatureSupported: " + y4.n.a(org.chromium.support_lib_boundary.util.Features.MULTI_PROFILE));
        try {
            return new com.tencent.xweb.sys.r();
        } catch (java.lang.Exception e17) {
            by5.c4.c("SysWebFactory", "getWebviewStorage error, sys WebStorage#getInstance error: " + e17);
            xx5.l.a(4, 1, z17);
            return null;
        }
    }
}
