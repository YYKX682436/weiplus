package com.tencent.mm.plugin.lite.jsapi.comms;

@kotlin.Metadata(d1 = {"\u0000\f\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0003\u0018\u00002\u00020\u0001B\u0007¢\u0006\u0004\b\u0002\u0010\u0003¨\u0006\u0004"}, d2 = {"Lcom/tencent/mm/plugin/lite/jsapi/comms/f4;", "Lcom/tencent/mm/plugin/lite/api/p;", "<init>", "()V", "luggage-lite-app_release"}, k = 1, mv = {1, 9, 0})
/* loaded from: classes8.dex */
public final class f4 extends com.tencent.mm.plugin.lite.api.p {
    public final void A(long j17, java.lang.String str, java.lang.String str2, java.lang.String str3, java.lang.String str4, java.lang.String str5) {
        com.tencent.mm.pluginsdk.model.g2 g2Var = new com.tencent.mm.pluginsdk.model.g2(str5, str2);
        g2Var.f189311d = str3;
        g2Var.f189310c = str4;
        g2Var.f189312e = j17;
        g2Var.f189313f = str;
        g2Var.f189315h = 3;
        jt.v vVar = (jt.v) i95.n0.c(jt.v.class);
        android.content.Context c17 = c();
        kotlin.jvm.internal.o.e(c17, "null cannot be cast to non-null type android.app.Activity");
        ((ht.a) vVar).getClass();
        com.tencent.mm.pluginsdk.ui.otherway.u0 u0Var = new com.tencent.mm.pluginsdk.ui.otherway.u0((android.app.Activity) c17, g2Var, null);
        fb0.k kVar = (fb0.k) i95.n0.c(fb0.k.class);
        android.content.Context c18 = c();
        kotlin.jvm.internal.o.f(c18, "getContext(...)");
        ((gb0.n) kVar).wi(c18, new com.tencent.mm.plugin.lite.jsapi.comms.e4(u0Var, this));
    }

    @Override // jd.b
    public void a(java.lang.String str, org.json.JSONObject data, boolean z17) {
        kotlin.jvm.internal.o.g(data, "data");
        try {
            java.lang.String string = data.getString("messageSvrId");
            kotlin.jvm.internal.o.f(string, "getString(...)");
            long parseLong = java.lang.Long.parseLong(string);
            java.lang.String string2 = data.getString("chatName");
            java.lang.String optString = data.optString("extInfo", "");
            com.tencent.mm.storage.f9 l17 = pt0.f0.f358209b1.l(string2, parseLong);
            if (l17 == null) {
                throw new java.lang.IllegalStateException("msgInfo".toString());
            }
            kotlin.jvm.internal.o.d(optString);
            if (optString.length() > 0) {
                jz5.f0 f0Var = null;
                java.lang.Object obj = com.tencent.mm.sdk.platformtools.aa.d(l17.j(), "msg", null).get(".msg.appmsg.recorditem");
                if (obj == null) {
                    throw new java.lang.IllegalStateException("Required value was null.".toString());
                }
                ((zs3.z) ((mc0.k) i95.n0.c(mc0.k.class))).getClass();
                r45.gp0 n17 = bt3.g2.n((java.lang.String) obj, optString);
                if (n17 != null) {
                    mc0.k kVar = (mc0.k) i95.n0.c(mc0.k.class);
                    java.lang.String Q0 = l17.Q0();
                    long msgId = l17.getMsgId();
                    ((zs3.z) kVar).getClass();
                    java.lang.String t17 = bt3.g2.t(n17, Q0, msgId, 1);
                    kotlin.jvm.internal.o.f(t17, "getDataPath(...)");
                    java.lang.String str2 = n17.K;
                    if (str2 == null) {
                        throw new java.lang.IllegalStateException("fileExt".toString());
                    }
                    java.lang.String str3 = n17.f375404d;
                    if (str3 == null) {
                        throw new java.lang.IllegalStateException(com.tencent.tmassistantsdk.downloadservice.DownloadInfo.FILENAME.toString());
                    }
                    ((ht.a) ((jt.v) i95.n0.c(jt.v.class))).getClass();
                    java.lang.String e17 = com.tencent.mm.pluginsdk.ui.tools.f.e(str2);
                    if (e17 == null) {
                        throw new java.lang.IllegalStateException("mimeType".toString());
                    }
                    long msgId2 = l17.getMsgId();
                    kotlin.jvm.internal.o.d(string2);
                    A(msgId2, string2, t17, str2, str3, e17);
                    this.f143442e.d();
                    f0Var = jz5.f0.f302826a;
                }
                if (f0Var == null) {
                    throw new java.lang.RuntimeException("findFavDataItem failed");
                }
                return;
            }
            com.tencent.mm.pluginsdk.model.app.d J0 = ((com.tencent.mm.pluginsdk.model.app.e) ((kt.q0) ((lt.q0) i95.n0.c(lt.q0.class))).wi()).J0(l17.getMsgId(), string2);
            if (J0 == null) {
                throw new java.lang.IllegalStateException("appAttachInfo".toString());
            }
            java.lang.String str4 = J0.field_fileFullPath;
            if (str4 == null) {
                throw new java.lang.IllegalStateException(com.tencent.tmassistantsdk.openSDK.QQDownloader.AssistantStore.DownloadInfos.DownloadInfoColumns.FILEPATH.toString());
            }
            java.lang.String n18 = com.tencent.mm.vfs.w6.n(str4);
            if (n18 == null) {
                throw new java.lang.IllegalStateException("fileExt".toString());
            }
            com.tencent.mm.vfs.z7 a17 = com.tencent.mm.vfs.z7.a(str4);
            java.lang.String str5 = a17.f213279f;
            if (str5 != null) {
                java.lang.String l18 = com.tencent.mm.vfs.e8.l(str5, false, false);
                if (!str5.equals(l18)) {
                    a17 = new com.tencent.mm.vfs.z7(a17.f213277d, a17.f213278e, l18, a17.f213280g, a17.f213281h);
                }
            }
            java.lang.String str6 = a17.f213279f;
            int lastIndexOf = str6.lastIndexOf("/");
            if (lastIndexOf >= 0) {
                str6 = str6.substring(lastIndexOf + 1);
            }
            java.lang.String str7 = str6;
            if (str7 == null) {
                throw new java.lang.IllegalStateException(com.tencent.tmassistantsdk.downloadservice.DownloadInfo.FILENAME.toString());
            }
            ((ht.a) ((jt.v) i95.n0.c(jt.v.class))).getClass();
            java.lang.String e18 = com.tencent.mm.pluginsdk.ui.tools.f.e(n18);
            if (e18 == null) {
                throw new java.lang.IllegalStateException("mimeType".toString());
            }
            long msgId3 = l17.getMsgId();
            kotlin.jvm.internal.o.d(string2);
            A(msgId3, string2, str4, n18, str7, e18);
            this.f143442e.d();
        } catch (java.lang.Exception e19) {
            com.tencent.mars.xlog.Log.e("MicroMsg.LiteAppJsApiOpenMessageFileToThirdApp", e19.getMessage() + ": " + jz5.a.b(e19));
            this.f143442e.a();
        }
    }
}
