package com.tencent.mm.plugin.finder.feed.ui;

/* loaded from: classes2.dex */
public final class a1 implements android.view.View.OnClickListener {

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ com.tencent.mm.plugin.finder.feed.ui.FinderBaseFeedUI f109641d;

    public a1(com.tencent.mm.plugin.finder.feed.ui.FinderBaseFeedUI finderBaseFeedUI) {
        this.f109641d = finderBaseFeedUI;
    }

    @Override // android.view.View.OnClickListener
    public final void onClick(android.view.View view) {
        java.util.ArrayList arrayList = new java.util.ArrayList();
        arrayList.add(view);
        java.lang.Object[] array = arrayList.toArray();
        arrayList.clear();
        yj0.a.b("com/tencent/mm/plugin/finder/feed/ui/FinderBaseFeedUI$fixActionBarStatus$2", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V", this, array);
        com.tencent.mm.plugin.finder.feed.ui.FinderBaseFeedUI finderBaseFeedUI = this.f109641d;
        finderBaseFeedUI.f128814p = false;
        finderBaseFeedUI.onBackPressed();
        finderBaseFeedUI.f128814p = true;
        yj0.a.h(this, "com/tencent/mm/plugin/finder/feed/ui/FinderBaseFeedUI$fixActionBarStatus$2", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V");
    }
}
