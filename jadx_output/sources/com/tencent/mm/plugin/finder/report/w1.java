package com.tencent.mm.plugin.finder.report;

/* loaded from: classes2.dex */
public final class w1 extends com.tencent.mm.plugin.finder.report.x1 {
    public w1(fc2.c cVar) {
        super(cVar);
        new android.graphics.Rect();
    }

    @Override // com.tencent.mm.plugin.finder.report.x1, fc2.o
    public fc2.a c(androidx.recyclerview.widget.RecyclerView recyclerView, int i17) {
        int i18;
        kotlin.jvm.internal.o.g(recyclerView, "recyclerView");
        fc2.y yVar = (fc2.y) super.c(recyclerView, i17);
        yVar.f261012j = hc2.f1.m(recyclerView, yVar.f261009g, yVar.f261010h);
        int i19 = this.f260960e;
        int i27 = yVar.f261009g;
        if (i19 != i27 || this.f260961f != yVar.f261010h) {
            if (i19 == Integer.MAX_VALUE || (i18 = this.f260961f) == Integer.MAX_VALUE) {
                i18 = yVar.f261010h;
            } else {
                if (i27 < i19) {
                    i19 = i27;
                }
                int i28 = yVar.f261010h;
                if (i28 > i18) {
                    i18 = i28;
                }
                i27 = i19;
            }
            yVar.f261011i = hc2.f1.m(recyclerView, i27, i18);
        }
        this.f260960e = yVar.f261009g;
        this.f260961f = yVar.f261010h;
        return yVar;
    }

    @Override // com.tencent.mm.plugin.finder.report.x1
    public fc2.a h(int i17) {
        return new fc2.y(i17);
    }

    @Override // com.tencent.mm.plugin.finder.report.x1
    /* renamed from: i */
    public fc2.y h(int i17) {
        return new fc2.y(i17);
    }

    @Override // com.tencent.mm.plugin.finder.report.x1
    public jz5.l j(androidx.recyclerview.widget.RecyclerView recyclerView, boolean z17) {
        int w17;
        int y17;
        kotlin.jvm.internal.o.g(recyclerView, "recyclerView");
        androidx.recyclerview.widget.RecyclerView.LayoutManager layoutManager = recyclerView.getLayoutManager();
        if (layoutManager instanceof androidx.recyclerview.widget.StaggeredGridLayoutManager) {
            androidx.recyclerview.widget.StaggeredGridLayoutManager staggeredGridLayoutManager = (androidx.recyclerview.widget.StaggeredGridLayoutManager) layoutManager;
            int[] u17 = staggeredGridLayoutManager.u(null);
            java.util.ArrayList arrayList = new java.util.ArrayList();
            int length = u17.length;
            y17 = 0;
            for (int i17 = 0; i17 < length; i17++) {
                int i18 = u17[i17];
                if (i18 >= 0) {
                    arrayList.add(java.lang.Integer.valueOf(i18));
                }
            }
            java.lang.Integer num = (java.lang.Integer) kz5.n0.o0(arrayList);
            w17 = num != null ? num.intValue() : 0;
            java.lang.Integer h07 = kz5.z.h0(staggeredGridLayoutManager.v(null));
            if (h07 != null) {
                y17 = h07.intValue();
            }
        } else {
            if (!(layoutManager instanceof androidx.recyclerview.widget.LinearLayoutManager)) {
                return new jz5.l(-1, -1);
            }
            androidx.recyclerview.widget.LinearLayoutManager linearLayoutManager = (androidx.recyclerview.widget.LinearLayoutManager) layoutManager;
            w17 = linearLayoutManager.w();
            y17 = linearLayoutManager.y();
        }
        return new jz5.l(java.lang.Integer.valueOf(k(recyclerView, z17, w17, 1)), java.lang.Integer.valueOf(k(recyclerView, z17, y17, -1)));
    }

    public final int k(androidx.recyclerview.widget.RecyclerView recyclerView, boolean z17, int i17, int i18) {
        if (i17 < 0) {
            return i17;
        }
        androidx.recyclerview.widget.f2 adapter = recyclerView.getAdapter();
        boolean z18 = false;
        if (i17 >= (adapter != null ? adapter.getItemCount() : 0) || !z17) {
            return i17;
        }
        android.graphics.Rect rect = new android.graphics.Rect();
        androidx.recyclerview.widget.k3 q07 = recyclerView.q0(i17, false);
        android.view.View view = q07 != null ? q07.itemView : null;
        if (view != null && view.getGlobalVisibleRect(rect)) {
            z18 = true;
        }
        return (!z18 || rect.height() < view.getHeight() / 2) ? k(recyclerView, z17, i17 + i18, i18) : i17;
    }
}
