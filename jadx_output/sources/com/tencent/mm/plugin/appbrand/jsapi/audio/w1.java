package com.tencent.mm.plugin.appbrand.jsapi.audio;

/* loaded from: classes8.dex */
public class w1 implements java.lang.Runnable {
    public w1(com.tencent.mm.plugin.appbrand.jsapi.audio.JsApiPausePlayVoice$PausePlayVoice jsApiPausePlayVoice$PausePlayVoice) {
    }

    @Override // java.lang.Runnable
    public void run() {
        com.tencent.mm.autogen.events.ExtPlayerEvent extPlayerEvent = new com.tencent.mm.autogen.events.ExtPlayerEvent();
        extPlayerEvent.f54204g.f7324a = 4;
        extPlayerEvent.e();
    }
}
