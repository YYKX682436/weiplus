package qe5;

/* loaded from: classes12.dex */
public final class s0 extends kotlin.jvm.internal.q implements yz5.l {

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ qe5.w0 f362189d;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public s0(qe5.w0 w0Var) {
        super(1);
        this.f362189d = w0Var;
    }

    /* JADX WARN: Can't fix incorrect switch cases order, some code will duplicate */
    @Override // yz5.l
    public java.lang.Object invoke(java.lang.Object obj) {
        java.lang.String str;
        java.lang.String str2;
        java.lang.String str3;
        android.view.MenuItem item = (android.view.MenuItem) obj;
        kotlin.jvm.internal.o.g(item, "item");
        int itemId = item.getItemId();
        qe5.w0 w0Var = this.f362189d;
        w0Var.getClass();
        jz5.f0 f0Var = jz5.f0.f302826a;
        boolean z17 = true;
        r5 = null;
        jz5.f0 f0Var2 = null;
        if (itemId == 215) {
            kd5.e Y6 = w0Var.Y6();
            java.lang.String str4 = Y6 != null ? Y6.F : null;
            com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
            am.dj djVar = localCheckResUpdateCacheFileEvent.f54480g;
            djVar.f6471a = 73;
            djVar.f6472b = 1;
            djVar.f6473c = str4;
            localCheckResUpdateCacheFileEvent.e();
            w0Var.Z6("apply success");
        } else if (itemId != 216) {
            switch (itemId) {
                case 25:
                    kd5.e Y62 = w0Var.Y6();
                    java.lang.String str5 = Y62 != null ? Y62.F : null;
                    java.lang.String str6 = "current template is " + com.tencent.mm.plugin.websearch.l2.a(0);
                    com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent2 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
                    am.dj djVar2 = localCheckResUpdateCacheFileEvent2.f54480g;
                    djVar2.f6471a = 27;
                    djVar2.f6472b = 1;
                    djVar2.f6473c = str5;
                    localCheckResUpdateCacheFileEvent2.e();
                    w0Var.Z6(str6);
                    com.tencent.mm.xwebutil.s0.b("com.tencent.mm.intent.ACTION_KILL_TOOLS_PROCESS");
                    break;
                case 26:
                    kd5.e Y63 = w0Var.Y6();
                    java.lang.String str7 = Y63 != null ? Y63.F : null;
                    java.lang.String str8 = "current wxa template is " + com.tencent.mm.plugin.websearch.l2.a(3);
                    com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent3 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
                    am.dj djVar3 = localCheckResUpdateCacheFileEvent3.f54480g;
                    djVar3.f6471a = 40;
                    djVar3.f6472b = 1;
                    djVar3.f6473c = str7;
                    localCheckResUpdateCacheFileEvent3.e();
                    w0Var.Z6(str8);
                    break;
                case 27:
                    kd5.e Y64 = w0Var.Y6();
                    java.lang.String str9 = Y64 != null ? Y64.F : null;
                    com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent checkResUpdateCacheFileEvent = new com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent();
                    am.r2 r2Var = checkResUpdateCacheFileEvent.f54053g;
                    r2Var.f7759a = 35;
                    r2Var.f7760b = 1;
                    r2Var.f7761c = str9;
                    checkResUpdateCacheFileEvent.e();
                    w0Var.Z6("apply success");
                    break;
                case 28:
                    kd5.e Y65 = w0Var.Y6();
                    java.lang.String str10 = Y65 != null ? Y65.F : null;
                    java.lang.String str11 = "current browse template is " + com.tencent.mm.plugin.websearch.l2.a(1);
                    com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent4 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
                    am.dj djVar4 = localCheckResUpdateCacheFileEvent4.f54480g;
                    djVar4.f6471a = 27;
                    djVar4.f6472b = 2;
                    djVar4.f6473c = str10;
                    localCheckResUpdateCacheFileEvent4.e();
                    w0Var.Z6(str11);
                    break;
                default:
                    switch (itemId) {
                        case 210:
                            kd5.e Y66 = w0Var.Y6();
                            java.lang.String str12 = Y66 != null ? Y66.F : null;
                            com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent checkResUpdateCacheFileEvent2 = new com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent();
                            am.r2 r2Var2 = checkResUpdateCacheFileEvent2.f54053g;
                            r2Var2.f7759a = 62;
                            r2Var2.f7760b = 1;
                            r2Var2.f7761c = str12;
                            checkResUpdateCacheFileEvent2.e();
                            w0Var.Z6("apply success");
                            break;
                        case 211:
                            com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent checkResUpdateCacheFileEvent3 = new com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent();
                            am.r2 r2Var3 = checkResUpdateCacheFileEvent3.f54053g;
                            r2Var3.f7759a = 62;
                            r2Var3.f7760b = 2;
                            r2Var3.f7761c = null;
                            checkResUpdateCacheFileEvent3.e();
                            w0Var.Z6("apply success");
                            break;
                        case 212:
                            kd5.e Y67 = w0Var.Y6();
                            java.lang.String str13 = Y67 != null ? Y67.F : null;
                            java.lang.String str14 = "current pardus template is " + com.tencent.mm.plugin.websearch.l2.a(5);
                            com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent5 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
                            am.dj djVar5 = localCheckResUpdateCacheFileEvent5.f54480g;
                            djVar5.f6471a = 66;
                            djVar5.f6472b = 1;
                            djVar5.f6473c = str13;
                            localCheckResUpdateCacheFileEvent5.e();
                            w0Var.Z6(str14);
                            break;
                        case 213:
                            com.tencent.mars.xlog.Log.i("MicroMsg.File.AppAttachApplyToUIC", "applyToScanGoods");
                            kd5.e Y68 = w0Var.Y6();
                            java.lang.String str15 = Y68 != null ? Y68.F : null;
                            com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent checkResUpdateCacheFileEvent4 = new com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent();
                            am.r2 r2Var4 = checkResUpdateCacheFileEvent4.f54053g;
                            r2Var4.f7759a = 64;
                            r2Var4.f7760b = 1;
                            r2Var4.f7761c = str15;
                            checkResUpdateCacheFileEvent4.e();
                            w0Var.Z6("apply success");
                            break;
                        default:
                            java.lang.String str16 = "";
                            switch (itemId) {
                                case 218:
                                    com.tencent.mars.xlog.Log.i("MicroMsg.File.AppAttachApplyToUIC", "applyToImageOcr");
                                    kd5.e Y69 = w0Var.Y6();
                                    java.lang.String str17 = Y69 != null ? Y69.F : null;
                                    com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent checkResUpdateCacheFileEvent5 = new com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent();
                                    am.r2 r2Var5 = checkResUpdateCacheFileEvent5.f54053g;
                                    r2Var5.f7759a = 84;
                                    r2Var5.f7760b = 1;
                                    r2Var5.f7761c = str17;
                                    checkResUpdateCacheFileEvent5.e();
                                    w0Var.Z6("apply success");
                                    break;
                                case 219:
                                    w0Var.V6(87, 1);
                                    break;
                                case 220:
                                    kd5.e Y610 = w0Var.Y6();
                                    java.lang.String str18 = Y610 != null ? Y610.F : null;
                                    com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent6 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
                                    am.dj djVar6 = localCheckResUpdateCacheFileEvent6.f54480g;
                                    djVar6.f6471a = 86;
                                    djVar6.f6472b = 2;
                                    djVar6.f6473c = str18;
                                    localCheckResUpdateCacheFileEvent6.e();
                                    w0Var.Z6("apply to TextStatus");
                                    break;
                                case com.tencent.thumbplayer.api.TPOptionalID.OPTION_ID_BEFORE_BOOL_VIDEO_ENABLE_MEDIA_CODEC_RELEASE_OUTPUT_BUFFER_WITH_TIMESTAMP /* 221 */:
                                    w0Var.V6(92, 1);
                                    break;
                                case com.tencent.cloud.huiyan.exception.CallbackErrorCode.HY_VERIFY_LOCAL_TIME_OUT /* 222 */:
                                    w0Var.V6(92, 2);
                                    break;
                                case 223:
                                    kd5.e Y611 = w0Var.Y6();
                                    java.lang.String str19 = Y611 != null ? Y611.F : null;
                                    java.lang.String str20 = "current ftshotsearch template is " + com.tencent.mm.plugin.websearch.l2.a(9);
                                    com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent7 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
                                    am.dj djVar7 = localCheckResUpdateCacheFileEvent7.f54480g;
                                    djVar7.f6471a = 95;
                                    djVar7.f6472b = 1;
                                    djVar7.f6473c = str19;
                                    localCheckResUpdateCacheFileEvent7.e();
                                    w0Var.Z6(str20);
                                    break;
                                case 224:
                                    w0Var.U6(true);
                                    break;
                                case 225:
                                    w0Var.U6(false);
                                    break;
                                case com.tencent.mm.plugin.appbrand.jsapi.contact.j.CTRL_INDEX /* 226 */:
                                    w0Var.W6("");
                                    break;
                                case 227:
                                    w0Var.W6("wx9e221f7828fa7482");
                                    break;
                                case 228:
                                    kd5.e Y612 = w0Var.Y6();
                                    java.lang.String str21 = Y612 != null ? Y612.F : null;
                                    java.lang.String str22 = "current emojisearch template is " + com.tencent.mm.plugin.websearch.l2.a(10);
                                    com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent8 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
                                    am.dj djVar8 = localCheckResUpdateCacheFileEvent8.f54480g;
                                    djVar8.f6471a = 79;
                                    djVar8.f6472b = 2;
                                    djVar8.f6473c = str21;
                                    localCheckResUpdateCacheFileEvent8.e();
                                    w0Var.Z6(str22);
                                    break;
                                default:
                                    switch (itemId) {
                                        case 230:
                                            w0Var.X6();
                                            break;
                                        case 231:
                                            w0Var.W6("wx2f3fb5db9f226462");
                                            break;
                                        case 232:
                                            w0Var.W6("wxe208ce76dfa39515");
                                            break;
                                        case com.tencent.cloud.huiyan.exception.CallbackErrorCode.HY_API_RESPONSE_ERROR /* 233 */:
                                            w0Var.T6();
                                            break;
                                        case 234:
                                            com.tencent.mars.xlog.Log.i("MicroMsg.File.AppAttachApplyToUIC", "applyToSelectText");
                                            kd5.e Y613 = w0Var.Y6();
                                            java.lang.String str23 = Y613 != null ? Y613.F : null;
                                            com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent checkResUpdateCacheFileEvent6 = new com.tencent.mm.autogen.events.CheckResUpdateCacheFileEvent();
                                            am.r2 r2Var6 = checkResUpdateCacheFileEvent6.f54053g;
                                            r2Var6.f7759a = 84;
                                            r2Var6.f7760b = 2;
                                            r2Var6.f7761c = str23;
                                            checkResUpdateCacheFileEvent6.e();
                                            w0Var.Z6("apply success");
                                            break;
                                        case 235:
                                            w0Var.W6("wxf337cbaa27790mb2");
                                            break;
                                        case com.tencent.mm.plugin.appbrand.jsapi.j6.CTRL_INDEX /* 236 */:
                                            w0Var.W6("MagicAdPublicService");
                                            break;
                                        case 237:
                                            w0Var.W6("MagicBrandService");
                                            break;
                                        case 238:
                                            w0Var.W6("MagicAdBrandService");
                                            break;
                                        case 239:
                                            kd5.e Y614 = w0Var.Y6();
                                            if (Y614 != null && (str = Y614.F) != null) {
                                                java.lang.String D = lp0.b.D();
                                                java.lang.String str24 = D + "runtime_package.zip";
                                                kotlin.jvm.internal.o.d(D);
                                                try {
                                                    if (!com.tencent.mm.vfs.w6.j(D)) {
                                                        com.tencent.mm.vfs.w6.u(D);
                                                    }
                                                    java.lang.String concat = D.concat("runtime_package.zip");
                                                    com.tencent.mm.vfs.w6.h(concat);
                                                    com.tencent.mm.vfs.w6.c(str, concat);
                                                } catch (java.lang.Exception e17) {
                                                    com.tencent.mars.xlog.Log.e("MicroMsg.File.AppAttachApplyToUIC", "performFileOperation error: " + e17.getMessage());
                                                    z17 = false;
                                                }
                                                if (z17) {
                                                    com.tencent.xweb.a3.n(w0Var.getContext(), str24);
                                                    com.tencent.mm.vfs.w6.h(str24);
                                                }
                                                if (z17) {
                                                    com.tencent.mars.xlog.Log.i("MicroMsg.File.AppAttachApplyToUIC", "applyToXWebRuntime success");
                                                } else {
                                                    com.tencent.mars.xlog.Log.w("MicroMsg.File.AppAttachApplyToUIC", "applyToXWebRuntime failed");
                                                }
                                                f0Var2 = f0Var;
                                            }
                                            if (f0Var2 == null) {
                                                com.tencent.mars.xlog.Log.w("MicroMsg.File.AppAttachApplyToUIC", "applyToXWebRuntime, no attach info");
                                                break;
                                            }
                                            break;
                                        case 240:
                                            w0Var.W6("MagicFinderCard");
                                            break;
                                        case 241:
                                            w0Var.W6("MagicSclNativeDemo");
                                            break;
                                        case 242:
                                            w0Var.W6("MagicAdPlayableBasic");
                                            break;
                                        case 243:
                                            w0Var.W6("MagicGameLoading");
                                            break;
                                        case 244:
                                            w0Var.W6(null);
                                            w0Var.X6();
                                            break;
                                        case 245:
                                            w0Var.W6("MagicEmojiCocos");
                                            break;
                                        case 246:
                                            w0Var.W6("MagicNewAdPlayableBasic");
                                            break;
                                        case 247:
                                            w0Var.W6("MagicEcsKFDynamicCard");
                                            break;
                                        case 248:
                                            w0Var.W6("MagicEcsTimeline");
                                            break;
                                        case com.tencent.mm.plugin.appbrand.jsapi.cd.CTRL_INDEX /* 249 */:
                                            w0Var.W6("MagicEcsPublicService");
                                            break;
                                        case 250:
                                            w0Var.W6("MagicBrushDemo");
                                            break;
                                        case 251:
                                            java.lang.StringBuilder sb6 = new java.lang.StringBuilder();
                                            sb6.append(((com.tencent.mm.plugin.magicbrush.s7) ((com.tencent.mm.plugin.magicbrush.s4) i95.n0.c(com.tencent.mm.plugin.magicbrush.s4.class))).Di());
                                            sb6.append("mbpkgs/");
                                            kd5.e Y615 = w0Var.Y6();
                                            if (Y615 == null || (str2 = Y615.f306892w) == null) {
                                                str2 = "";
                                            }
                                            sb6.append(str2);
                                            if (!com.tencent.mm.vfs.w6.h(sb6.toString())) {
                                                w0Var.Z6("No debug pkg found");
                                                break;
                                            } else {
                                                kd5.e Y616 = w0Var.Y6();
                                                if (Y616 != null && (str3 = Y616.f306892w) != null) {
                                                    str16 = str3;
                                                }
                                                ((he3.e) ((ie3.i) i95.n0.c(ie3.i.class))).wi(r26.n0.r0(str16, '.', null, 2, null));
                                                w0Var.Z6("Done");
                                                break;
                                            }
                                            break;
                                        case 252:
                                            w0Var.W6("MagicEcsShop");
                                            break;
                                    }
                            }
                    }
            }
        } else {
            kd5.e Y617 = w0Var.Y6();
            java.lang.String str25 = Y617 != null ? Y617.F : null;
            com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent localCheckResUpdateCacheFileEvent9 = new com.tencent.mm.autogen.events.LocalCheckResUpdateCacheFileEvent();
            am.dj djVar9 = localCheckResUpdateCacheFileEvent9.f54480g;
            djVar9.f6471a = 79;
            djVar9.f6472b = 1;
            djVar9.f6473c = str25;
            localCheckResUpdateCacheFileEvent9.e();
            w0Var.Z6("apply success");
            w0Var.T6();
        }
        return f0Var;
    }
}
