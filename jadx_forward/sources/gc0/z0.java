package gc0;

/* loaded from: classes5.dex */
public final class z0 {
    public z0(p3321xbce91901.jvm.p3324x21ffc6bd.i iVar) {
    }

    public final void a(java.util.Map map, int i17, r45.kl5 data) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(map, "map");
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(data, "data");
        int b17 = b(data);
        if (i17 != 1) {
            if (i17 != 2) {
                if (i17 != 3) {
                    return;
                }
                map.put("has_candidate_photo", 1);
                map.put("candidate_photo_source", java.lang.Integer.valueOf(b17));
                return;
            }
            java.lang.String m75945x2fec8307 = data.m75945x2fec8307(1);
            if (m75945x2fec8307 == null || r26.n0.N(m75945x2fec8307)) {
                return;
            }
            map.put("has_candidate_phone", 1);
            map.put("candidate_phone_source", java.lang.Integer.valueOf(b17));
            return;
        }
        java.lang.String m75945x2fec83072 = data.m75945x2fec8307(0);
        if (m75945x2fec83072 == null || r26.n0.N(m75945x2fec83072)) {
            return;
        }
        long m75942xfb822ef2 = data.m75942xfb822ef2(3);
        if (m75942xfb822ef2 == 1) {
            map.put("has_candidate_cn", 1);
            map.put("candidate_cn_source", java.lang.Integer.valueOf(b17));
            return;
        }
        if (m75942xfb822ef2 == 2) {
            map.put("has_candidate_en", 1);
            map.put("candidate_en_source", java.lang.Integer.valueOf(b17));
        } else if (m75942xfb822ef2 == 3) {
            map.put("has_candidate_position", 1);
            map.put("candidate_position_source", java.lang.Integer.valueOf(b17));
        } else if (m75942xfb822ef2 == 4) {
            map.put("has_candidate_institution", 1);
            map.put("candidate_institution_source", java.lang.Integer.valueOf(b17));
        }
    }

    public final int b(r45.kl5 kl5Var) {
        int i17;
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(kl5Var, "<this>");
        r45.ll5 ll5Var = (r45.ll5) kl5Var.m75936x14adae67(5);
        if (ll5Var == null) {
            return 0;
        }
        int i18 = 1;
        jz5.l lVar = new jz5.l(java.lang.Integer.valueOf(ll5Var.m75939xb282bd08(0)), java.lang.Integer.valueOf(ll5Var.m75939xb282bd08(1)));
        if (!p3321xbce91901.jvm.p3324x21ffc6bd.o.b(lVar, new jz5.l(1, 0))) {
            if (p3321xbce91901.jvm.p3324x21ffc6bd.o.b(lVar, new jz5.l(1, 1))) {
                return 2;
            }
            if (p3321xbce91901.jvm.p3324x21ffc6bd.o.b(lVar, new jz5.l(2, 0))) {
                return 3;
            }
            i18 = 4;
            if (!p3321xbce91901.jvm.p3324x21ffc6bd.o.b(lVar, new jz5.l(2, 1))) {
                if (p3321xbce91901.jvm.p3324x21ffc6bd.o.b(lVar, new jz5.l(0, 2))) {
                    i17 = 6;
                } else if (p3321xbce91901.jvm.p3324x21ffc6bd.o.b(lVar, new jz5.l(0, 4))) {
                    i17 = 7;
                } else {
                    if (!p3321xbce91901.jvm.p3324x21ffc6bd.o.b(lVar, new jz5.l(0, 3))) {
                        return 0;
                    }
                    i17 = 8;
                }
                return i17;
            }
        }
        return i18;
    }
}
