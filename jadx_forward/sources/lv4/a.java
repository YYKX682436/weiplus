package lv4;

/* loaded from: classes12.dex */
public final class a implements android.view.View.OnClickListener {

    /* renamed from: d, reason: collision with root package name */
    public final /* synthetic */ lv4.b f403044d;

    public a(lv4.b bVar) {
        this.f403044d = bVar;
    }

    @Override // android.view.View.OnClickListener
    public final void onClick(android.view.View view) {
        java.util.ArrayList arrayList = new java.util.ArrayList();
        arrayList.add(view);
        java.lang.Object[] array = arrayList.toArray();
        arrayList.clear();
        yj0.a.b("com/tencent/mm/plugin/websearch/teach/base/item/FTSTeachBoxTitleItem$bindData$1", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V", this, array);
        lv4.b bVar = this.f403044d;
        jv4.i iVar = bVar.f403047c.f482090c;
        if (iVar != null) {
            ((jv4.e) ((jv4.p) iVar).f383795e).f383768a.l();
        }
        qv4.r0 r0Var = (qv4.r0) bVar.f403046b;
        r0Var.f448553n = true;
        java.util.ArrayList arrayList2 = new java.util.ArrayList();
        rv4.b0 b0Var = r0Var.f448545f;
        if (b0Var != null) {
            java.util.List list = b0Var.f481975e;
            java.util.ArrayList arrayList3 = new java.util.ArrayList(kz5.d0.q(list, 10));
            java.util.Iterator it = ((java.util.ArrayList) list).iterator();
            while (it.hasNext()) {
                rv4.c0 c0Var = (rv4.c0) ((mv4.j) it.next());
                java.util.Map l17 = kz5.c1.l(new jz5.l(com.p314xaae8f345.mm.p1006xc5476f33.p1873x468d4ffd.p1875x75a49f33.C16532xe69b9d82.f37660xb6ae3b28, c0Var.f481982c), new jz5.l("visible", java.lang.Boolean.valueOf(c0Var.b())), new jz5.l("settingType", 2));
                java.lang.Long l18 = c0Var.f481983d;
                if (l18 != null) {
                    l17.put("isSwitchOn", java.lang.Long.valueOf(l18.longValue()));
                }
                java.lang.Boolean bool = c0Var.f481987h;
                if (bool != null) {
                    l17.put("isSwitchClosed", java.lang.Boolean.valueOf(bool.booleanValue()));
                }
                java.lang.Integer num = c0Var.f481984e;
                if (num != null) {
                    l17.put("businessType", java.lang.Integer.valueOf(num.intValue()));
                }
                org.json.JSONObject a17 = ri.l0.a(l17);
                arrayList3.add(a17 != null ? java.lang.Boolean.valueOf(arrayList2.add(a17)) : null);
            }
        }
        com.p314xaae8f345.mm.p1006xc5476f33.p1797x32b0ae.s sVar = new com.p314xaae8f345.mm.p1006xc5476f33.p1797x32b0ae.s();
        sVar.f225700d = new qv4.n0(r0Var);
        org.json.JSONObject a18 = ri.l0.a(kz5.c1.k(new jz5.l("sourceFrom", 1), new jz5.l(com.p314xaae8f345.p362xadfe2b3.p372x8fb0427b.C3712x3ed8a441.f14354xe2e3e984, r0Var.f448540a.f413207a), new jz5.l("verticalSettingItems", arrayList2)));
        q80.g0 g0Var = (q80.g0) i95.n0.c(q80.g0.class);
        com.p314xaae8f345.mm.ui.AbstractActivityC21394xb3d2c0cf abstractActivityC21394xb3d2c0cf = r0Var.f448541b;
        android.os.Bundle bundle = new android.os.Bundle();
        bundle.putString("appId", "wxalite292c2ce0ae0ce2b6ff62226ac60ed4b4");
        bundle.putString(com.p314xaae8f345.p347x615374d.p349x208df1fe.p358xd1075a44.p360x696c9db.C3365xfbc5fb5e.f13305x20d82714, "pages/seach-vertical-setting/entry");
        bundle.putBoolean("forbidRightGesture", true);
        bundle.putString("minVersion", "2.0.9");
        bundle.putDouble("heightPercent", 0.67d);
        bundle.putBoolean("isHalfScreen", true);
        bundle.putString("query", java.lang.String.valueOf(a18));
        ((com.p314xaae8f345.mm.p689xc5a27af6.p753x32b0ae.i) g0Var).jk(abstractActivityC21394xb3d2c0cf, bundle, true, false, sVar, new qv4.o0());
        yj0.a.h(this, "com/tencent/mm/plugin/websearch/teach/base/item/FTSTeachBoxTitleItem$bindData$1", "android/view/View$OnClickListener", "onClick", "(Landroid/view/View;)V");
    }
}
