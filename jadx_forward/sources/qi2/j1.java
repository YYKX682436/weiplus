package qi2;

/* loaded from: classes10.dex */
public final class j1 extends com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g implements android.view.View.OnClickListener {
    public final kj2.o A;

    /* renamed from: h, reason: collision with root package name */
    public final zh2.c f445068h;

    /* renamed from: i, reason: collision with root package name */
    public android.view.View f445069i;

    /* renamed from: m, reason: collision with root package name */
    public android.view.View f445070m;

    /* renamed from: n, reason: collision with root package name */
    public android.view.View f445071n;

    /* renamed from: o, reason: collision with root package name */
    public com.p314xaae8f345.mm.ui.p2747xd1075a44.p2757xb099fd48.k0 f445072o;

    /* renamed from: p, reason: collision with root package name */
    public android.widget.TextView f445073p;

    /* renamed from: q, reason: collision with root package name */
    public android.widget.TextView f445074q;

    /* renamed from: r, reason: collision with root package name */
    public android.view.View f445075r;

    /* renamed from: s, reason: collision with root package name */
    public android.widget.TextView f445076s;

    /* renamed from: t, reason: collision with root package name */
    public android.view.View f445077t;

    /* renamed from: u, reason: collision with root package name */
    public android.widget.TextView f445078u;

    /* renamed from: v, reason: collision with root package name */
    public com.p314xaae8f345.mm.ui.p2747xd1075a44.p2760xcbb7be80.C22699x3dcdb352 f445079v;

    /* renamed from: w, reason: collision with root package name */
    public android.view.View f445080w;

    /* renamed from: x, reason: collision with root package name */
    public final jz5.g f445081x;

    /* renamed from: y, reason: collision with root package name */
    public final nj2.p f445082y;

    /* renamed from: z, reason: collision with root package name */
    public final mj2.k f445083z;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public j1(android.content.Context context, zh2.e userInterface, zh2.c pluginAbility) {
        super(context, false, null, false, 12, null);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(context, "context");
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(userInterface, "userInterface");
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(pluginAbility, "pluginAbility");
        this.f445068h = pluginAbility;
        this.f445081x = jz5.h.b(new qi2.i1(this, context));
        android.view.View view = this.f445080w;
        if (view == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("inviteListLayout");
            throw null;
        }
        this.A = new kj2.o(view, pluginAbility.g(), new qi2.y0(this), null, new qi2.z0(this));
        android.view.ViewGroup.LayoutParams layoutParams = this.f199917g.f526370d.getLayoutParams();
        if (layoutParams != null) {
            android.view.View view2 = this.f445069i;
            if (view2 == null) {
                p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rootView");
                throw null;
            }
            layoutParams.height = (com.p314xaae8f345.mm.ui.bl.b(view2.getContext()).y * 3) / 4;
        }
        gk2.e g17 = pluginAbility.g();
        android.view.View view3 = this.f445069i;
        if (view3 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rootView");
            throw null;
        }
        this.f445083z = new mj2.k(context, g17, view3, pluginAbility, userInterface, new qi2.x0(this));
        android.view.View view4 = this.f445069i;
        if (view4 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rootView");
            throw null;
        }
        android.view.View findViewById = view4.findViewById(com.p314xaae8f345.mm.R.id.qv7);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById, "findViewById(...)");
        this.f445082y = new nj2.p(view4, findViewById, pluginAbility.g(), userInterface);
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g
    public int j() {
        return com.p314xaae8f345.mm.R.C30864xbddafb2a.dle;
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g
    public int l() {
        return 8;
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g
    public android.view.View m(android.view.ViewGroup root, zl2.u4 uiMode) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(root, "root");
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(uiMode, "uiMode");
        zl2.w4 w4Var = zl2.w4.f555550a;
        zl2.u4 u4Var = zl2.u4.f555523f;
        android.content.Context context = this.f199914d;
        return w4Var.b(com.p314xaae8f345.mm.R.C30864xbddafb2a.dle, root, false, u4Var, context, com.p314xaae8f345.mm.ui.id.b(context));
    }

    /* JADX WARN: Removed duplicated region for block: B:26:0x0107  */
    @Override // android.view.View.OnClickListener
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct add '--show-bad-code' argument
    */
    public void onClick(android.view.View r14) {
        /*
            Method dump skipped, instructions count: 473
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: qi2.j1.onClick(android.view.View):void");
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g
    public void p(android.view.View rootView) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(rootView, "rootView");
        this.f445069i = rootView;
        android.view.View findViewById = rootView.findViewById(com.p314xaae8f345.mm.R.id.hrl);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById, "findViewById(...)");
        this.f445070m = findViewById;
        android.view.View findViewById2 = rootView.findViewById(com.p314xaae8f345.mm.R.id.f568623qx4);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById2, "findViewById(...)");
        this.f445071n = findViewById2;
        android.view.View findViewById3 = rootView.findViewById(com.p314xaae8f345.mm.R.id.hqq);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById3, "findViewById(...)");
        this.f445073p = (android.widget.TextView) findViewById3;
        android.view.View findViewById4 = rootView.findViewById(com.p314xaae8f345.mm.R.id.hqp);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById4, "findViewById(...)");
        this.f445074q = (android.widget.TextView) findViewById4;
        android.view.View findViewById5 = rootView.findViewById(com.p314xaae8f345.mm.R.id.f6c);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById5, "findViewById(...)");
        this.f445075r = findViewById5;
        android.view.View findViewById6 = rootView.findViewById(com.p314xaae8f345.mm.R.id.fkk);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById6, "findViewById(...)");
        this.f445076s = (android.widget.TextView) findViewById6;
        android.view.View findViewById7 = rootView.findViewById(com.p314xaae8f345.mm.R.id.qvb);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById7, "findViewById(...)");
        this.f445077t = findViewById7;
        android.view.View findViewById8 = rootView.findViewById(com.p314xaae8f345.mm.R.id.f568624qx5);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById8, "findViewById(...)");
        this.f445078u = (android.widget.TextView) findViewById8;
        android.view.View findViewById9 = rootView.findViewById(com.p314xaae8f345.mm.R.id.f568622qx3);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById9, "findViewById(...)");
        this.f445079v = (com.p314xaae8f345.mm.ui.p2747xd1075a44.p2760xcbb7be80.C22699x3dcdb352) findViewById9;
        android.view.View findViewById10 = rootView.findViewById(com.p314xaae8f345.mm.R.id.qtx);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(findViewById10, "findViewById(...)");
        this.f445080w = findViewById10;
        android.view.View view = this.f445070m;
        if (view == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("changeModeLayout");
            throw null;
        }
        view.setOnClickListener(this);
        android.view.View view2 = this.f445071n;
        if (view2 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("startPkLayout");
            throw null;
        }
        view2.setOnClickListener(this);
        android.view.View view3 = this.f445075r;
        if (view3 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("leftLayout");
            throw null;
        }
        view3.setOnClickListener(this);
        android.widget.TextView textView = this.f445076s;
        if (textView == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rightTextView");
            throw null;
        }
        textView.setOnClickListener(this);
        android.widget.TextView textView2 = this.f445073p;
        if (textView2 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("headerTitleText");
            throw null;
        }
        com.p314xaae8f345.mm.ui.bk.r0(textView2.getPaint(), 0.8f);
        android.widget.TextView textView3 = this.f445076s;
        if (textView3 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rightTextView");
            throw null;
        }
        com.p314xaae8f345.mm.ui.bk.r0(textView3.getPaint(), 0.8f);
        android.widget.TextView textView4 = this.f445074q;
        if (textView4 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("battleStatusText");
            throw null;
        }
        com.p314xaae8f345.mm.ui.bk.r0(textView4.getPaint(), 0.8f);
        ((com.p314xaae8f345.mm.ui.p2747xd1075a44.p2760xcbb7be80.C22699x3dcdb352) rootView.findViewById(com.p314xaae8f345.mm.R.id.f6b)).m82040x7541828(this.f199914d.getResources().getColor(com.p314xaae8f345.mm.R.C30859x5a72f63.f560639q5));
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g
    public void t() {
        super.t();
        if (y()) {
            this.A.b();
        }
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g
    public void v() {
        dk2.u4 u4Var = ((mm2.o4) this.f445068h.g().a(mm2.o4.class)).Z;
        boolean z17 = false;
        if (u4Var != null && u4Var.h()) {
            z17 = true;
        }
        android.content.Context context = this.f199914d;
        fj2.j jVar = fj2.j.f344707a;
        fj2.s sVar = fj2.s.f344716a;
        if (z17) {
            sVar.a(ml2.k1.f409177n);
            com.p314xaae8f345.mm.ui.p2747xd1075a44.p2757xb099fd48.k0 k0Var = this.f445072o;
            java.lang.String string = context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.f573009m35);
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(string, "getString(...)");
            this.f445072o = jVar.b(k0Var, context, string, new qi2.d1(this));
            return;
        }
        sVar.a(ml2.k1.f409176m);
        com.p314xaae8f345.mm.ui.p2747xd1075a44.p2757xb099fd48.k0 k0Var2 = this.f445072o;
        java.lang.String string2 = context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.jdz);
        p3321xbce91901.jvm.p3324x21ffc6bd.o.f(string2, "getString(...)");
        this.f445072o = jVar.b(k0Var2, context, string2, new qi2.f1(this));
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1543xd1075a44.g
    public void w() {
        jz5.f0 f0Var;
        super.w();
        android.widget.TextView textView = this.f445076s;
        if (textView == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rightTextView");
            throw null;
        }
        android.content.Context context = this.f199914d;
        textView.setText(context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.jdz));
        android.widget.TextView textView2 = this.f445076s;
        if (textView2 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rightTextView");
            throw null;
        }
        textView2.setVisibility(0);
        android.view.View view = this.f445077t;
        if (view == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("playOptionLayout");
            throw null;
        }
        java.util.ArrayList arrayList = new java.util.ArrayList();
        java.lang.ThreadLocal threadLocal = zj0.c.f554818a;
        arrayList.add(0);
        java.util.Collections.reverse(arrayList);
        yj0.a.d(view, arrayList.toArray(), "com/tencent/mm/plugin/finder/live/mic/pk/FinderLiveVisitorPkPanelWidget", "openPanel", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
        view.setVisibility(((java.lang.Integer) arrayList.get(0)).intValue());
        yj0.a.f(view, "com/tencent/mm/plugin/finder/live/mic/pk/FinderLiveVisitorPkPanelWidget", "openPanel", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
        zh2.c cVar = this.f445068h;
        dk2.u4 u4Var = ((mm2.o4) cVar.g().a(mm2.o4.class)).Z;
        if (u4Var != null) {
            if (u4Var.h()) {
                if (((mm2.e1) cVar.g().a(mm2.e1.class)).c7()) {
                    android.widget.TextView textView3 = this.f445076s;
                    if (textView3 == null) {
                        p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rightTextView");
                        throw null;
                    }
                    textView3.setText(context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.f573009m35));
                } else {
                    android.widget.TextView textView4 = this.f445076s;
                    if (textView4 == null) {
                        p3321xbce91901.jvm.p3324x21ffc6bd.o.o("rightTextView");
                        throw null;
                    }
                    textView4.setVisibility(8);
                }
                android.view.View view2 = this.f445077t;
                if (view2 == null) {
                    p3321xbce91901.jvm.p3324x21ffc6bd.o.o("playOptionLayout");
                    throw null;
                }
                java.util.ArrayList arrayList2 = new java.util.ArrayList();
                arrayList2.add(8);
                java.util.Collections.reverse(arrayList2);
                yj0.a.d(view2, arrayList2.toArray(), "com/tencent/mm/plugin/finder/live/mic/pk/FinderLiveVisitorPkPanelWidget", "showRightTextView", "(Lcom/tencent/mm/plugin/finder/live/model/FinderLiveBattleData;)V", "android/view/View_EXEC_", "setVisibility", "(I)V");
                view2.setVisibility(((java.lang.Integer) arrayList2.get(0)).intValue());
                yj0.a.f(view2, "com/tencent/mm/plugin/finder/live/mic/pk/FinderLiveVisitorPkPanelWidget", "showRightTextView", "(Lcom/tencent/mm/plugin/finder/live/model/FinderLiveBattleData;)V", "android/view/View_EXEC_", "setVisibility", "(I)V");
            }
            if (u4Var.i()) {
                com.p314xaae8f345.mm.ui.p2747xd1075a44.p2760xcbb7be80.C22699x3dcdb352 c22699x3dcdb352 = this.f445079v;
                if (c22699x3dcdb352 == null) {
                    p3321xbce91901.jvm.p3324x21ffc6bd.o.o("startPkIcon");
                    throw null;
                }
                c22699x3dcdb352.setImageResource(com.p314xaae8f345.mm.R.raw.f79887x15f6a8c3);
                android.widget.TextView textView5 = this.f445078u;
                if (textView5 == null) {
                    p3321xbce91901.jvm.p3324x21ffc6bd.o.o("startPkTextView");
                    throw null;
                }
                textView5.setText(context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.d6v));
            } else {
                com.p314xaae8f345.mm.ui.p2747xd1075a44.p2760xcbb7be80.C22699x3dcdb352 c22699x3dcdb3522 = this.f445079v;
                if (c22699x3dcdb3522 == null) {
                    p3321xbce91901.jvm.p3324x21ffc6bd.o.o("startPkIcon");
                    throw null;
                }
                c22699x3dcdb3522.setImageResource(com.p314xaae8f345.mm.R.raw.f79865xc8527e4c);
                android.widget.TextView textView6 = this.f445078u;
                if (textView6 == null) {
                    p3321xbce91901.jvm.p3324x21ffc6bd.o.o("startPkTextView");
                    throw null;
                }
                textView6.setText(context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.lzs));
            }
            java.lang.String string = u4Var.f315693g == 1 ? context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.f573205dw1) : context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.f573207dw4);
            p3321xbce91901.jvm.p3324x21ffc6bd.o.d(string);
            java.lang.String string2 = u4Var.f315694h == 0 ? context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.ljg) : context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.lje);
            p3321xbce91901.jvm.p3324x21ffc6bd.o.d(string2);
            android.widget.TextView textView7 = this.f445073p;
            if (textView7 == null) {
                p3321xbce91901.jvm.p3324x21ffc6bd.o.o("headerTitleText");
                throw null;
            }
            textView7.setText(context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.lzr, string, string2));
            java.lang.String string3 = u4Var.f315691e == 2 ? context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.iqp) : context.getResources().getString(com.p314xaae8f345.mm.R.C30867xcad56011.lzt);
            p3321xbce91901.jvm.p3324x21ffc6bd.o.d(string3);
            android.widget.TextView textView8 = this.f445074q;
            if (textView8 == null) {
                p3321xbce91901.jvm.p3324x21ffc6bd.o.o("battleStatusText");
                throw null;
            }
            textView8.setText(string3);
            f0Var = jz5.f0.f384359a;
        } else {
            f0Var = null;
        }
        if (f0Var == null) {
            int i17 = rl.b.f478676a;
            com.p314xaae8f345.p542x3306d5.p550x382fcc.Log.e("VisitorPkPanelWidget", "curBattle is null", null);
        }
        android.view.View view3 = this.f445080w;
        if (view3 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("inviteListLayout");
            throw null;
        }
        java.util.ArrayList arrayList3 = new java.util.ArrayList();
        arrayList3.add(8);
        java.util.Collections.reverse(arrayList3);
        yj0.a.d(view3, arrayList3.toArray(), "com/tencent/mm/plugin/finder/live/mic/pk/FinderLiveVisitorPkPanelWidget", "openPanel", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
        view3.setVisibility(((java.lang.Integer) arrayList3.get(0)).intValue());
        yj0.a.f(view3, "com/tencent/mm/plugin/finder/live/mic/pk/FinderLiveVisitorPkPanelWidget", "openPanel", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
        if (y()) {
            this.A.f();
        }
        this.f445082y.n();
        this.f445083z.s();
    }

    public final boolean y() {
        zh2.c cVar = this.f445068h;
        return ((mm2.e1) cVar.g().a(mm2.e1.class)).c7() || ((mm2.c1) cVar.g().a(mm2.c1.class)).L;
    }

    public void z() {
        this.f445083z.r("onTimerUpdateData");
        nj2.p pVar = this.f445082y;
        pVar.getClass();
        pm0.v.X(new nj2.l(pVar));
        if (y()) {
            this.A.c();
        }
    }
}
