package ym5;

/* loaded from: classes15.dex */
public abstract class a1 {

    /* renamed from: a, reason: collision with root package name */
    public static final android.os.Handler f544722a = new android.os.Handler(android.os.Looper.getMainLooper());

    /* renamed from: b, reason: collision with root package name */
    public static final java.util.HashMap f544723b = new java.util.HashMap();

    /* renamed from: c, reason: collision with root package name */
    public static final java.util.HashMap f544724c = new java.util.HashMap();

    public static final void a(int i17, ym5.b1 b1Var) {
        boolean z17 = b1Var.f544744b;
        java.util.HashMap hashMap = f544724c;
        java.util.HashMap hashMap2 = f544723b;
        java.util.HashMap hashMap3 = z17 ? hashMap : hashMap2;
        if (hashMap3.containsKey(java.lang.Integer.valueOf(i17))) {
            return;
        }
        hashMap3.put(java.lang.Integer.valueOf(i17), b1Var);
        ym5.s0 s0Var = new ym5.s0(b1Var);
        android.view.View view = b1Var.f544743a;
        if (view.getTag(com.p314xaae8f345.mm.R.id.dkk) != null) {
            java.lang.Object tag = view.getTag(com.p314xaae8f345.mm.R.id.dkk);
            p3321xbce91901.jvm.p3324x21ffc6bd.o.e(tag, "null cannot be cast to non-null type android.view.View.OnAttachStateChangeListener");
            view.removeOnAttachStateChangeListener((android.view.View.OnAttachStateChangeListener) tag);
        }
        view.setTag(com.p314xaae8f345.mm.R.id.dkk, s0Var);
        view.addOnAttachStateChangeListener(s0Var);
        b1Var.f544746d = s0Var;
        ym5.r0 r0Var = ym5.r0.f545018a;
        ym5.r0.f545021d = hashMap2.size();
        ym5.r0.f545022e = hashMap.size();
    }

    public static final ym5.b1 b(android.view.View view) {
        return (ym5.b1) f544723b.get(java.lang.Integer.valueOf(view.hashCode()));
    }

    public static final ym5.b1 c(android.view.View view) {
        return (ym5.b1) f544724c.get(java.lang.Integer.valueOf(view.hashCode()));
    }

    public static final ym5.b1 d(int i17) {
        java.util.HashMap hashMap = f544723b;
        ym5.b1 b1Var = (ym5.b1) hashMap.get(java.lang.Integer.valueOf(i17));
        if (b1Var != null) {
            b1Var.f544745c = -1L;
            b1Var.f544746d = null;
            b1Var.f544752j = true;
        }
        java.util.HashMap hashMap2 = f544724c;
        ym5.b1 b1Var2 = (ym5.b1) hashMap2.get(java.lang.Integer.valueOf(i17));
        if (b1Var2 != null) {
            b1Var2.f544745c = -1L;
            b1Var2.f544746d = null;
            b1Var2.f544752j = true;
        }
        ym5.r0 r0Var = ym5.r0.f545018a;
        ym5.r0.f545021d = hashMap.size();
        ym5.r0.f545022e = hashMap2.size();
        return b1Var;
    }

    public static final android.util.Printer e(android.os.Looper looper) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(looper, "<this>");
        try {
            p3321xbce91901.C29043x91b2b43d.Companion companion = p3321xbce91901.C29043x91b2b43d.INSTANCE;
            java.lang.reflect.Field declaredField = android.os.Looper.class.getDeclaredField("mLogging");
            declaredField.setAccessible(true);
            java.lang.Object obj = declaredField.get(looper);
            if (obj instanceof android.util.Printer) {
                return (android.util.Printer) obj;
            }
            return null;
        } catch (java.lang.Throwable th6) {
            p3321xbce91901.C29043x91b2b43d.Companion companion2 = p3321xbce91901.C29043x91b2b43d.INSTANCE;
            p3321xbce91901.C29043x91b2b43d.m143895xf1229813(p3321xbce91901.C29044xefd6a286.m143914x452354ee(th6));
            return null;
        }
    }

    public static final void f(yz5.a call) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(call, "call");
        long id6 = java.lang.Thread.currentThread().getId();
        android.os.Handler handler = f544722a;
        if (id6 == handler.getLooper().getThread().getId()) {
            call.mo152xb9724478();
        } else {
            handler.post(new ym5.t0(call));
        }
    }

    public static final void g(p012xc85e97e9.p103xe821e364.p104xd1075a44.C1163xf1deaba4 c1163xf1deaba4, ym5.m0 m0Var) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(c1163xf1deaba4, "<this>");
        f(new ym5.w0(m0Var, c1163xf1deaba4));
    }

    public static final void h(android.view.View view, ym5.n0 n0Var) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(view, "<this>");
        f(new ym5.x0(n0Var, view));
    }

    public static final void i(android.view.View view, ym5.n0 n0Var) {
        p3321xbce91901.jvm.p3324x21ffc6bd.o.g(view, "<this>");
        f(new ym5.z0(n0Var, view));
    }
}
