package com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5;

/* loaded from: classes10.dex */
public final class j2 extends com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2 {

    /* renamed from: h, reason: collision with root package name */
    public final lj2.r0 f197933h;

    /* renamed from: i, reason: collision with root package name */
    public final nj2.p f197934i;

    /* renamed from: m, reason: collision with root package name */
    public final mj2.j f197935m;

    /* renamed from: n, reason: collision with root package name */
    public final android.view.View f197936n;

    /* renamed from: o, reason: collision with root package name */
    public final jz5.g f197937o;

    /* JADX WARN: Illegal instructions before constructor call */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct add '--show-bad-code' argument
    */
    public j2(gk2.e r12, android.view.View r13, zh2.e r14, zh2.c r15) {
        /*
            r11 = this;
            java.lang.String r0 = "liveData"
            p3321xbce91901.jvm.p3324x21ffc6bd.o.g(r12, r0)
            java.lang.String r0 = "root"
            p3321xbce91901.jvm.p3324x21ffc6bd.o.g(r13, r0)
            java.lang.String r0 = "userInterface"
            p3321xbce91901.jvm.p3324x21ffc6bd.o.g(r14, r0)
            java.lang.String r0 = "micLinkPluginAbility"
            p3321xbce91901.jvm.p3324x21ffc6bd.o.g(r15, r0)
            android.content.Context r0 = r13.getContext()
            java.lang.String r1 = "getContext(...)"
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(r0, r1)
            r11.<init>(r12, r0)
            com.tencent.mm.plugin.finder.live.view.i2 r0 = new com.tencent.mm.plugin.finder.live.view.i2
            r0.<init>(r11, r12, r15)
            jz5.g r0 = jz5.h.b(r0)
            r11.f197937o = r0
            android.content.Context r0 = r11.getContext()
            r2 = 2131495283(0x7f0c0973, float:1.8614098E38)
            android.view.View r0 = android.view.View.inflate(r0, r2, r11)
            lj2.r0 r2 = new lj2.r0
            android.content.Context r3 = r13.getContext()
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(r3, r1)
            p3321xbce91901.jvm.p3324x21ffc6bd.o.d(r0)
            r2.<init>(r3, r12, r0, r15)
            r11.f197933h = r2
            r2 = 2131310814(0x7f0938de, float:1.823995E38)
            android.view.View r2 = r0.findViewById(r2)
            java.lang.String r10 = "findViewById(...)"
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(r2, r10)
            r11.f197936n = r2
            mj2.j r2 = new mj2.j
            android.content.Context r4 = r13.getContext()
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(r4, r1)
            com.tencent.mm.plugin.finder.live.view.f2 r9 = new com.tencent.mm.plugin.finder.live.view.f2
            r9.<init>(r11)
            r3 = r2
            r5 = r12
            r6 = r0
            r7 = r15
            r8 = r14
            r3.<init>(r4, r5, r6, r7, r8, r9)
            r11.f197935m = r2
            nj2.p r15 = new nj2.p
            r1 = 2131315759(0x7f094c2f, float:1.824998E38)
            android.view.View r0 = r0.findViewById(r1)
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(r0, r10)
            r15.<init>(r13, r0, r12, r14)
            r11.f197934i = r15
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.j2.<init>(gk2.e, android.view.View, zh2.e, zh2.c):void");
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: getInviteWidget */
    public final kj2.o m57598x7849dd23() {
        return (kj2.o) ((jz5.n) this.f197937o).mo141623x754a37bb();
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void a(int i17) {
        boolean z17 = this.f197966f == i17;
        this.f197965e = z17;
        if (z17) {
            q();
        } else {
            p();
        }
        if (i17 == 0) {
            fj2.s.c(fj2.s.f344716a, ml2.q2.f409368v, "1", null, 0, 0, 28, null);
        }
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void e() {
        in0.q e17;
        zn0.a0 U;
        this.f197967g = true;
        if (this.f197965e) {
            q();
        }
        lj2.r0 r0Var = this.f197933h;
        zh2.c cVar = r0Var.f400461g;
        boolean z17 = ((mm2.c1) cVar.g().a(mm2.c1.class)).S4;
        android.view.View view = r0Var.f400463i;
        if (z17) {
            if (view != null) {
                java.util.ArrayList arrayList = new java.util.ArrayList();
                java.lang.ThreadLocal threadLocal = zj0.c.f554818a;
                arrayList.add(0);
                java.util.Collections.reverse(arrayList);
                yj0.a.d(view, arrayList.toArray(), "com/tencent/mm/plugin/finder/live/mic/widget/FinderLiveSetMicModeWidget", "onBind", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
                view.setVisibility(((java.lang.Integer) arrayList.get(0)).intValue());
                yj0.a.f(view, "com/tencent/mm/plugin/finder/live/mic/widget/FinderLiveSetMicModeWidget", "onBind", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
            }
            ((lj2.n) ((jz5.n) r0Var.f400468q).mo141623x754a37bb()).b();
            int i17 = (((qf2.l) cVar.mo57008x143f1b92(qf2.l.class)) == null || (e17 = dk2.ef.f314905a.e()) == null || (U = e17.U()) == null) ? 1 : U.f555850u;
            r0Var.f400462h = i17;
            r0Var.b(i17);
            com.p314xaae8f345.p542x3306d5.p550x382fcc.Log.i("FinderLiveSetMicModeWidget", "onBind: " + z17 + ", curMicModeType: " + r0Var.f400462h);
        } else {
            if (view != null) {
                java.util.ArrayList arrayList2 = new java.util.ArrayList();
                java.lang.ThreadLocal threadLocal2 = zj0.c.f554818a;
                arrayList2.add(8);
                java.util.Collections.reverse(arrayList2);
                yj0.a.d(view, arrayList2.toArray(), "com/tencent/mm/plugin/finder/live/mic/widget/FinderLiveSetMicModeWidget", "onBind", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
                view.setVisibility(((java.lang.Integer) arrayList2.get(0)).intValue());
                yj0.a.f(view, "com/tencent/mm/plugin/finder/live/mic/widget/FinderLiveSetMicModeWidget", "onBind", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
            }
            com.p314xaae8f345.p542x3306d5.p550x382fcc.Log.e("FinderLiveSetMicModeWidget", "onBind: " + z17);
        }
        this.f197934i.n();
        this.f197935m.r("onBind");
        android.view.View view2 = this.f197936n;
        if (view2 == null) {
            p3321xbce91901.jvm.p3324x21ffc6bd.o.o("inviteListLayout");
            throw null;
        }
        java.util.ArrayList arrayList3 = new java.util.ArrayList();
        java.lang.ThreadLocal threadLocal3 = zj0.c.f554818a;
        arrayList3.add(8);
        java.util.Collections.reverse(arrayList3);
        yj0.a.d(view2, arrayList3.toArray(), "com/tencent/mm/plugin/finder/live/view/FinderLiveApplyLinkNewVisitorView", "onBind", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
        view2.setVisibility(((java.lang.Integer) arrayList3.get(0)).intValue());
        yj0.a.f(view2, "com/tencent/mm/plugin/finder/live/view/FinderLiveApplyLinkNewVisitorView", "onBind", "()V", "android/view/View_EXEC_", "setVisibility", "(I)V");
        if (o()) {
            m57598x7849dd23().f();
        }
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void f() {
        this.f197967g = false;
        if (this.f197965e) {
            p();
        }
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void g(boolean z17, int i17) {
        com.p314xaae8f345.p542x3306d5.p550x382fcc.Log.i("Finder.FinderLiveApplyLinkVisitorView", "keyboardChange show:" + z17 + " height:" + i17);
        this.f197934i.o(z17, i17);
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void h(long j17) {
        this.f197934i.p(j17);
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void i(gk2.e eVar) {
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void j() {
        this.f197934i.q();
        this.f197935m.r("onNotifyDataChanged");
        if (o()) {
            m57598x7849dd23().c();
        }
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void k(boolean z17, km2.q qVar) {
        this.f197934i.r(z17, qVar);
        this.f197935m.o(z17, qVar);
        if (o()) {
            m57598x7849dd23().c();
        }
    }

    @Override // com.p314xaae8f345.mm.p1006xc5476f33.p1418xb4097826.p1473x32b0ec.p1534x373aa5.k2
    public void l(gk2.e eVar) {
        nj2.p pVar = this.f197934i;
        pVar.getClass();
        pm0.v.X(new nj2.l(pVar));
        this.f197935m.r("onTimerUpdateData");
        if (o()) {
            m57598x7849dd23().c();
        }
    }

    public final boolean o() {
        return (((mm2.e1) m57780xd052fc2c().a(mm2.e1.class)).c7() || ((mm2.c1) m57780xd052fc2c().a(mm2.c1.class)).L) && this.f197967g;
    }

    public final void p() {
        if (this.f197933h.f400469r) {
            org.json.JSONObject jSONObject = new org.json.JSONObject();
            ml2.g3[] g3VarArr = ml2.g3.f409011d;
            jSONObject.put("type", 1);
            i95.m c17 = i95.n0.c(ml2.j0.class);
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(c17, "getService(...)");
            zy2.zb.y6((zy2.zb) c17, ml2.z4.O2, jSONObject.toString(), null, 4, null);
        }
    }

    public final void q() {
        lj2.r0 r0Var = this.f197933h;
        if (((mm2.c1) r0Var.f400461g.g().a(mm2.c1.class)).S4) {
            org.json.JSONObject jSONObject = new org.json.JSONObject();
            ml2.u3[] u3VarArr = ml2.u3.f409643d;
            jSONObject.put("type", 1);
            i95.m c17 = i95.n0.c(ml2.j0.class);
            p3321xbce91901.jvm.p3324x21ffc6bd.o.f(c17, "getService(...)");
            zy2.zb.y6((zy2.zb) c17, ml2.z4.N2, jSONObject.toString(), null, 4, null);
        }
        r0Var.f400469r = false;
    }
}
