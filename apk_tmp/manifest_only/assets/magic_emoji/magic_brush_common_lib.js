...
(function (global) {
  ...
  const _log = function (level, args, logToVConsole) {
    const logData = getNativeGlobalLog(level, args);
    JSBridge.invoke('systemLog', { level: _levelMap[level] ?? 'debug', content: logData, enableVConsole: logToVConsole ? logToVConsole : global.mb.enableVConsole });
  };
  global._log = _log;
})(this);
...
(function(global) {
  ...
  global.setImmediate = undefined; // hook setImmediate because of bug in node.js
...
//    console.log('on subscribe event: ', event, JSON.stringify(result))
  ...
  var jsLog = function (level, args) {
    // fallback to jsLog(args)
    if (typeof level === 'string') {
      global._log(1, [level]);
    } else {
      global._log(level, [args], false);
    }
  };
  global.JSBridge = {
    invoke: invoke,
