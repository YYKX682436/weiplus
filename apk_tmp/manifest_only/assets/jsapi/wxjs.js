...
(function () {
  ...
  ;(function($){
      var _WXJS_MediaTracker = (function() {
        var lastPlayedElementRef = null // 存储最近播放的音频元素
        var hasLastPlayedElement = false

        function getLastPlayedElement() {
          if (!lastPlayedElementRef) {
            return null
          }
          var element = lastPlayedElementRef.deref();
        ...
        function isLiveStream(element) {
          ...
          try {
            if (element.hasAttribute('data-live') ||
                element.hasAttribute('data-stream') ||
                element.hasAttribute('live-stream')) {
              _log('live stream detected: ' + element.getAttribute('data-live') +
                  element.getAttribute('data-stream') +
                  element.getAttribute('live-stream'));
              return true;
            }

            if (element.duration === Infinity || element.duration === Number.POSITIVE_INFINITY) {
              _log('live stream detected: ' + element.duration);
              return true;
            }
        ...
        function handleMediaEvent(e) {
            ...
            if (target.tagName === 'AUDIO') {
                switch (e.type) {
                    case 'play':
                        lastPlayedElementRef = new WeakRef(target); // 存储弱引用
                        hasLastPlayedElement = true
                        break;
                    case 'ended':
                        break;
                }
                ...
                // 处理视频事件
                switch (e.type) {
                    case 'playing':// 发现在部分场景下，play事件触发时duration还未更新（NAN）使用 Play 判断不够准确或没有触发 play
                        ...
                        _sendMessage(JSON.stringify({
                                  ...
                                  '__fullApiName': 'reportSecureAction'
                                }));
                        _call('reportSecureAction', data);
                        break;
                    case 'pause':
                    case 'ended':
                        break;
        ...
        mediaEvents.forEach(function(eventType) {
          document.addEventListener(eventType, handleMediaEvent, true);
        });
        return {
          // 恢复上次播放的媒体
          recoverLastPlayback: function() {
            var lastPlayedElement = getLastPlayedElement();
            if (!lastPlayedElement) {
              return Promise.resolve(); // 元素已经被释放
            }
            try {
                if (lastPlayedElement.paused) {
                    return lastPlayedElement.play();
                }
            } catch (e) {
                // console.warn('播放恢复失败:', e);
                return Promise.reject(e);
            }
            // console.log('no lastPlayedElement')
            return Promise.resolve();
          },

          pauseLastPlayback: function() {
            var lastPlayedElement = getLastPlayedElement();
            if (!lastPlayedElement) {
              return Promise.resolve(); // 元素已经被释放
            }
            try {
                if (!lastPlayedElement.paused) {
                    return lastPlayedElement.pause();
                }
            } catch (e) {
                // console.warn('播放暂停失败:', e);
                return Promise.reject(e);
            }
            return Promise.resolve();
          },

          isLastPlaybackEnded: function() {
            var lastPlayedElement = getLastPlayedElement();
            if (!lastPlayedElement && hasLastPlayedElement) {
              return true // 元素已经被释放
            }
            try {
                return !!(
                    lastPlayedElement &&
                    (lastPlayedElement.readyState > 0) && // 确保已加载元数据
                    (lastPlayedElement.ended ||
                      lastPlayedElement.currentTime >= lastPlayedElement.duration)
                );
            } catch(e) {
                return false; // 元素可能已被移除DOM
            }
          },

          isPlaying: function() {
            var lastPlayedElement = getLastPlayedElement();
            if (!lastPlayedElement) return false; // 元素已经被释放
              try {
                  return !!(
                      lastPlayedElement &&
                      !lastPlayedElement.paused &&
                      !lastPlayedElement.ended &&
                      lastPlayedElement.currentTime < lastPlayedElement.duration
                  );
              } catch(e) {
      ...
      // 将接口挂载到_WXJS，调用方式：_WXJS.mediaTracker.recoverLastPlayback()
      $.mediaTracker = _WXJS_MediaTracker;
    })(_WXJS)
  __initLog(__DL, "media tracker init");
  ...
  var _runOn3rdApiList = [], //可以在第三方网页上运行的api列表
    _event_hook_map_for3rd = {}, //第三方网页hook的事件表 'event' => callback
     _event_subscribe_hook_map_for3rd = {},
  ...
  var _subscribeFor3rdIdentifier = _subscribeFor3rd;
      ...
      case 'event':
        {
          if (typeof msgWrap[_EVENT_ID] === 'string') {
            var eventHook = _event_hook_map[msgWrap[_EVENT_ID]];
            var thirdEventHook = _event_hook_map_for3rd[msgWrap[_EVENT_ID]];
            var thirdSubscribeHooks = _event_subscribe_hook_map_for3rd[msgWrap[_EVENT_ID]];
            var eventPayload = msgWrap['__params'];
            var isIn3rdApiList = _isIn3rdApiList(msgWrap[_EVENT_ID]);

            if (typeof thirdEventHook === 'function' && isIn3rdApiList) {
              var shareEvents = ['menu:share:timeline', 'menu:share:qq', 'menu:share:QZone', 'menu:share:appmessage', 'menu:share:email']
              ...
              var ret = thirdEventHook(eventPayload);
              return JSON.stringify(ret);
            } else if (Array.isArray(thirdSubscribeHooks) && thirdSubscribeHooks.length && isIn3rdApiList) {
              var retList = []
              for (var hookIndex = 0; hookIndex < thirdSubscribeHooks.length; hookIndex ++) {
                  if (typeof thirdSubscribeHooks[hookIndex] === 'function') {
                      var result = thirdSubscribeHooks[hookIndex](eventPayload)
                      retList.push(result)
                  }
              }
              return JSON.stringify(retList)

            } else if (typeof eventHook === 'function') {
              ret = eventHook(eventPayload);
              return JSON.stringify(ret);
            }
  ...
  function _call(func, params, callback) {
    ...
    if (curFuncIdentifier !== _callIdentifier) {
      _log('_call error: curFuncIdentifier !== _callIdentifier curFuncIdentifier: ' + curFuncIdentifier + ' _callIdentifier: ' + _callIdentifier);
      return;
    }
    if (!func || typeof func !== 'string') {
      _log('_call error: func is not string func: ' + func + '');
      return;
    };
  ...
  function _subscribeFor3rd(event, callback) {
    var curFuncIdentifier = __WeixinJSBridge.subscribe;
    if (curFuncIdentifier !== _subscribeFor3rdIdentifier) {
        return;
    }
    if (!event || typeof event !== 'string') {
        return;
    ...
    if (_event_subscribe_hook_map_for3rd[event]) {
        _event_subscribe_hook_map_for3rd[event].push(callback);
    } else {
        _event_subscribe_hook_map_for3rd[event] = [callback];
    };
  }


    function _unsubscribeFor3rd(event, callback) {
        var subscribedCallBacks = _event_subscribe_hook_map_for3rd[event];

        if (subscribedCallBacks && subscribedCallBacks.length) {
            _event_subscribe_hook_map_for3rd[event] = subscribedCallBacks.filter(function(cb) {
                return cb !== callback
            })
        }
    }
  ...
  function _setDefaultEventHandlers() {
    ...
    function getMetaTitle() {
        var element = document.head.querySelector('meta[property~="og:title"],meta[name~="og:title"]');
        return element && element.getAttribute("content");
    }

    function getMetaImage() {
        var element = document.head.querySelector('meta[property~="og:image"],meta[name~="og:image"]');
        return element && element.getAttribute("content");
    }

    // 为了兼容head中没有og:image的场景，不影响到现有的解析
    function getMetaExtraImage() {
        try {
            var element = document.querySelector('meta[property~="og:image"],meta[name~="og:image"]');
    ...
    // 不影响到现有的解析，增加og:description的解析
    function getMetaExtraDesc() {
        try {
            var element = document.querySelector('meta[property~="og:description"],meta[name~="og:description"]');
            if (element && element.getAttribute("content")) {
                return element.getAttribute("content");
            } else {
                return "";
    ...
    _on('menu:share:timeline', function (argv) {
      ...
      if (typeof argv.title === 'string') {
        ...
        data = {
          ...
          "desc": "",
          "title": getMetaTitle() || document.title,
          "img_url": getMetaImage()
        };
    ...
    _on('menu:share:appmessage', function (argv) {
      var data;
      if (typeof argv.title === 'string') {
        _log('share appmessage with jsapi');
        data = argv;
        // 申请临时分享权限
        _sendMessage(JSON.stringify({
          '__handleFromWeixin': _xxyy,
        ...
        _log('share appmessage raw');
        data = {
          "link": document.documentURI || _session_data.init_url,
          "desc": "",
          "title": getMetaTitle() || document.title,
          "img_url": getMetaImage(),
          "img_extra_url": getMetaExtraImage(),
          "extra_desc": getMetaExtraDesc()
        };
  ...
  var __WeixinJSBridge = {
    ...
    call: _call,
    on: _onfor3rd,
    subscribe: _subscribeFor3rd,
    unsubscribe: _unsubscribeFor3rd,
    env: _env,
    log: _log,
    state: _state,
    // private
